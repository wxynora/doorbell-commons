...
- React 19.2 与 Vite 8；
- Zod 4 作为共享运行时合同；
- SQLite 与 `better-sqlite3` 作为后续持久化切片的数据库基线；
- `@fastify/websocket` 作为后续实时切片的传输基线；
- Biome 2 负责格式化与静态检查；
- Node.js 内置测试运行器负责第一阶段后端定向测试。

技术栈的精确工程边界与当前实现状态见 `docs/runtime-architecture.md`。选定技术基线不表示对应业务能力已经实现；数据库、WebSocket、身份和生产部署仍需在各自施工任务中落地。
...
- 加入 QQ 群只代表具备申请资格，不自动创建居民、家园或 Connector。
- 群成员资格的具体验证技术尚未确定，不在产品方案中预设机器人协议、验证码或人工审核流程。
...
14. QQ 群 `515831305` 成员资格的第一版可信验证方式，以及成员状态变化的确认来源。
