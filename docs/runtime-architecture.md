...
## 休息室扭蛋旧回执读取边界（2026-09-23）

Farm 仍是抽取、扣款和幂等回执的唯一权威。Main 只以当前登录人绑定的农场身份和原请求编号调用 Farm 私有只读回执入口；该入口只读取既有回执，不执行抽取或重扣。十连正常重试继续使用原编号。旧浏览器若只保存通用 pending 编号，先读回执辨认单抽或十连；查不到时停止新抽取，由管理员核对。

## 农场三维装饰运行边界（2026-09-07）

- Main发布线提供Three.js田园场景、商店/背包UI、严格共享协议与鉴权代理；Farm发布线负责装饰商品、农场金币结算、持有数量、格子规则和SQLite持久化。两条线分别发布，不复用社区数据库存农场布局。
- 新数据位于农场`farmDecorations`，保存回执位于`doorbellHumanFarmDecorationReceipts`；旧牧场装饰保持隔离。存档提交后才替换内存状态，保存带版本与幂等键；客户端不能提交农场绑定身份。
- 季节、天气、灾害读取Farm自然权威，昼夜按服务端时间换算北京时间5～7点、17～19点过渡，跟随现有读取刷新。真实已解锁地块决定三维地块和禁止摆放区域。
- 本地检查页、模拟接口、测试与预览控制台不属于发布资源。已有5246个人预览及其浏览器布局不属于正式存档迁移范围。
...
symlink to that fixed `node_modules`. The three npm-workspace entries inside that fixed layer must be
absolute links back to the stable current-runtime paths for `packages/protocol`, `apps/server` and
`apps/web`; a missing, relative or mismatched workspace link rejects before service stop. A dependency
change likewise rejects rather than installing, rebuilding or copying dependencies. The approved PWA
