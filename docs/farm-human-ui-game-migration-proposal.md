...
> 状态：四界面、页面菜单归属、邻里首批范围与前后端边界已经确认；本文同时记录当前真实能力、目标接口和施工前置。只有明确写为“已接入”的能力才视为已经实现。
>
> 当前新 React 农场已接通 field、ranch、kitchen、catalog、bulletin、流光原野与铃野共行的严格结构化读取，并接回现有 farm 权威能够直接证明的 Human 写操作：帮收；牧场居民照料、收取、装饰、派遣与金币往来；料理商店食材、食谱、三件工具购买与刷新，真实库存烹饪及料理库存使用、回收、摆摊；原创植物、全部设置字段、探险、作物收藏、全服集市与跨户原子交易、邻里留言和熔炼。农场／牧场购物车只保存一次后台幂等快照并发出一次 Bell 通知，TA 仍自行使用现有 farm 购买工具。每条写链都使用权威 revision、UUID 幂等与真实 receipt，React 不复制玩法或本地结算。更完整邻里／牧场历史只在 farm 提供真实字段后显示。旧 `/api/farm/ui*` 仍只是迁移期 HTML 代理；本批新增链已经进入 Git，尚未部署或生产验证。
...
- 正式 `/lingye/farm` 入口和农场 React 模块均按点击动态加载，没有塞进社区首屏；
- 农场、牧场、料理台、邻里四个场景，以及播报、商店、通用工具、行动面板和料理库存面板已经形成独立 lazy JS／CSS 边界；未访问场景与未打开的大面板不在首页 HTML 中预载；
- 运行图片由稳定素材键和受控 source map 进入 Vite，构建时同时检查 manifest 声明、运行引用和内容目录，生成内容 hash URL；显式 `fallback／missing` 保留，`approved-source` 不进入构建输出；
- 社区唯一 Web App Manifest 与 Service Worker 已建立：导航 NetworkFirst、带 hash 的静态资源 CacheFirst、所有 `/api/**` NetworkOnly，不注册 Background Sync；
- 以上是本地构建事实，尚未部署，也未完成真实安全来源下的冷／热启动与 Cache Storage 验收，不能把本地打包通过写成上线完成。
...
| 熔炼 | `demo=full` 继续用四列素材格预览 30 种权威素材并按 `N → R → SR → SSR → SP` 排序；Live 只显示 `/api/farm/catalog` 返回的真实素材 ID、名称、等级和数量，恰好选中三项后才开放“开始熔炼”。提交只带三项稳定素材 ID、当前熔炼 revision 与 UUID 幂等键；farm 复用旧 Human `/craft` 同一 `engine.craft()` 权威链完成库存复核、扣料、配方／随机、种子发放与任务事件，成功一次保存并返回真实作物名、等级和新目录。Demo 结果只用于视觉预览，不进入 Live |
...
Doorbell 社区侧 `GET /api/farm/field` 与 `POST /api/farm/field/harvest-assists` 已完成严格 browser/internal schema、独立 farm Human service client、当前 Human session 绑定派生、`no-store` 与错误映射；浏览器不能提供或覆盖 Human key／门牌，也不能选择地块。farm 发布线已经新增纯投影 `/internal/doorbell/human/field/read` 与权威 `/internal/doorbell/human/field/harvest-assist`；React Live 完整保留 `revision／server_time`，显示权威金币、状态资料、地块和帮收摘要，并在成功后展示真实收获 receipt、整体替换新 field。该批 farm `440381a` 与 Doorbell main `004c013` 已分别推送，尚未部署或生产验证。

`GET /api/farm/catalog` 已提供真实熔炼素材与 `farm-smelting-v1` revision；`POST /api/farm/smelting/actions` 已从当前 Human session 派生农场身份并接到 farm `/internal/doorbell/human/smelting/action`。浏览器不能提交 Human key、门牌、概率、配方、产物或余额；同一次未知网络结果必须复用原 UUID 与三项素材，farm 只在 clone 上复用既有 `engine.craft()`，成功才一次保存。

Ranch／Kitchen 的旧 view 仍带有推进产出、初始化厨房、按日刷新商店、消费随机状态与保存等写语义，因此新只读接口没有直接包装它们。farm 侧现已分别建立纯 projector：`ranch/read` 只投影已经持久化的牧场余额、居民、产出、持有物、派遣与商店快照；`kitchen/read` 只投影已经持久化的余额、工具、堆叠食材、牧场产物／鱼获／财宝／料理实例、已知食谱与每日货架，并把未初始化或旧日货架明确标为 unavailable；`catalog/read` 同理只读取已经存在的农场菜单资源。三者不 advance、不初始化、不刷新、不结算、不消费随机数、不保存，未知 ID 不猜。牧场居民详情动作、牧场一键收取和三项设置保存现已各自通过独立 adapter 在 clone 上复用旧权威函数并一次原子保存；料理台整车购买同样只在 clone 上按请求顺序复用 `kitchenBuy`，任一项失败整车零变更，全部成功才一次保存。其余写操作仍须独立 action 合同，不能由 Doorbell 或 React 重算。
...
→ 在购物车内增减数量或移除
→ 点击「喊 TA 来买」
→ Doorbell 保存一份后台幂等快照并只通知自家 AI 一次
→ AI 收到商品清单后自行调用现有 farm 购买工具
→ Human 页面只确认“已通知 TA”，不跟踪或宣称购买结果
```
...
- 农场／牧场的“喊 TA 来买”只在整车条目能由当前真实货架解析时启用；受理后清车并显示“已通知 TA”，不显示请求编号、商品到账或购买成功。Live 料理台只有整车条目都能由当前权威货架解析时才启用“确认购买”。

农场／牧场额外规则：

- 购物车结算键必须叫“喊 TA 来买”或同义的请求文案，不能叫“支付成功”“已购买”；
- 提交成功只表示通知已由 Doorbell 接收，不能显示商品已经到账；
- 一车只保存一份后台请求快照和一次 wake，不按商品逐条唤醒；
- TA 收到通知后自行调用既有 farm 权威购买工具；Doorbell 不新增购买工具、逐项决定回传或第二套购买结算；
- 请求受理后清空当前购物车并显示“已通知 TA”，不保留请求状态卡、不轮询结果；网络结果不明时由同一个 `Idempotency-Key` 重试；
...
- farm 返回真实整车 receipt 后 UI 才整体替换 kitchen resource、清空购物车并显示实际件数、银币支出与余额；receipt 同时绑定门牌、幂等键、逐项商品／数量和当前货架版本。
- 整车只提交食材与每日食谱。farm 在 clone 上逐项复用权威购买校验，任一商品失败不保存、不扣银币，全部成功才保存一次；货架变化、状态冲突、购买拒绝、幂等冲突和服务／合同错误都返回明确失败。
- 网络结果不明时，UI 保留并复用原请求的完整购物车、版本和 UUID 幂等键；不得从主确认键另发新 key。货架变化或状态冲突重新读取权威商店并清空旧货架购物车，由 Human 重新选择。
- 三件付费工具尚缺逐户持有状态迁移，不进入本合同，也不得把前端草稿、点击确认或本地状态变化冒充为工具购买或料理成功。
...
请求不上传价格、货币、农场身份或 Human key。Doorbell 先重新读取当前商店并验证整车引用；快照已过期时返回 409 `shop_changed`，整车不创建、不唤醒，并让 UI 刷新商品后由人类重新确认。验证成功后只在后台保存 `purchase_request_id`、整车快照和一次 wake 关联，再通过已批准的铃生产者通知自家 AI。请求编号不进入通知正文、Bell payload 或 Human 页面。通知正文固定为 `【📢来自铃野的通知】\n你的人类{human_name}想要你给她买{shop_name}的{items_text}。`。Doorbell 不提供购买结果查询；真正购买仍由 TA 调用现有 farm 权威工具完成。料理台不复用该请求接口，继续使用已经接通的 Human 直接购买合同。
...
| `farm-panel-smelting` | 打开熔炼 | 当前真实素材图、数量、等级、三项选择与权威结果框 | 未打开时不加载全素材图；Live 不预制结果图或本地结算 |
...
- 探险与熔炼：5 张地图封面、30 种素材图、24 件探险装饰图；熔炼结果的名称、等级和状态由权威 receipt 与 DOM／CSS 展示，不把随机产物烤进图片；
...
| 农场菜单 | `GET /api/farm/catalog` 为商店、背包、作物图鉴、集市、探险、熔炼、设置与邻里提供无副作用结构化读取；`GET /api/farm/bulletin` 复用 field 纯投影读取成熟地块，并只读取存档中已接受且未完成的真实任务，留言与牧场通知仍来自现有持久化来源。原创植物、全部设置字段、探险、作物收藏和熔炼均已接通独立 farm 权威 action；网络结果未知复用同一 payload／revision／UUID，成功重读或替换权威资源 | 更完整邻里读取与历史只在 farm 提供真实字段后展示 |
| 牧场 | `GET /api/farm/ranch` 只渲染真实居民、余额、产出、持有物、派遣与商店。居民详情已接通投喂、升级、改名、置顶、配饰和已解锁异色；场景已接一键收取，背包装饰可摆放／收回，派遣面板可发起派遣、收取待结算往来并完成金币转入／转回；具体来访目标的抓捕继续走目标动作，不恢复全局管理条。牧场购物车按一次请求产生一次 Bell 通知，TA 仍使用现有 farm 购买工具 | 更完整访客、往来历史与巡逻鹅明细只在 farm 提供真实字段后展示 |
| 料理台 | `GET /api/farm/kitchen` 显示真实银币、逐户工具持有、堆叠食材与产物／鱼获／财宝／料理实例。食材、每日两道食谱及烤炉／蒸笼／炸锅整车购买、食材手动刷新，以及库存的本人／宠物使用、牧场产物／料理系统回收、料理摆摊、鱼获按量／全部出售和财宝按量回收均已接通权威 action，实例 ID 与锁定价值不被拍平。Live 烹饪只提交 2～5 个真实库存引用并使用 farm 返回的实际扣料、成品、解锁与新 kitchen resource | 90 道食谱仍缺权威 `method_id`、方式唯一性和工具要求；方式选择与一键制作不得在迁移前伪造 |
| 四页本地状态 | 农场／牧场／料理台／邻里四场景、播报、商店、通用工具、行动面板与料理库存面板均已形成按首次进入／打开加载的 lazy JS／CSS 边界；访问后的页面仍隐藏挂载，分类、面板和滚动位置在当前 React 会话内保留，不写浏览器持久化或权威状态 | 继续以真实冷／热网络验收确认没有意外预取；只有产品另行确认后才考虑跨刷新恢复 |
| 邻里 | 排行榜、留言板、原创作物三个平级栏目只显示当前选中内容且没有足迹。Live 从 catalog 安全投影读取真实条目；留言表单只从真实公开农场目录选择目标，提交成功显示 farm receipt 并重读目录，不本地伪造留言；无目标或来源 unavailable 时保持空态 | 补榜单细分、本人位置、完整公开资料、原创作物详情／收藏和更丰富留言读取；不扩展为假数据 |
...
| 商店／购物车 | 三页商店 Live 分别读取 farm catalog、ranch 与 kitchen 的真实持久化货架。料理台食材、每日食谱和三件付费工具按一车一次直接提交 farm，任一项失败整车零变更，成功才清车并显示真实 receipt；食材手动刷新也已接权威计数与费用。农场／牧场“喊 TA 来买”按一车一次创建后台幂等快照与一次 Bell 通知，受理后只显示“已通知 TA”；TA 自行调用既有 farm 购买工具，Human 不查询购买结果 | 只对已有合法 AI 购买动作的商品开放；不新增购买工具、结果回写或第二套结算 |
| 集市 | `/api/farm/catalog` 返回全服同一权威普通在售项和换物单；React 三页保留各自筛选／返回位置。浏览、上架、下架、创建／撤回换物挂单、他户普通购买及接受换物均已接通独立 action；普通项继续用卖家门牌＋kind＋item_id，换物单保留真实 listing ID。跨户动作只调用 farm 的双户原子提交，保存失败恢复买家、卖家与相关 UGC，不用连续单户保存冒充成功 | 等待两条发布线提交／部署和真实账号验收，不改变价格、手续费、库存或换物规则 |
| 熔炼 | React `demo=full` 已接线 30 种权威素材图集；Live 从 `/api/farm/catalog` 只显示接口实际返回的素材 ID、名称、等级和数量，目录按 `N → R → SR → SSR → SP`、每行四格排列。恰好选择三项后，`POST /api/farm/smelting/actions` 以当前 revision 和 UUID 幂等键调用 farm；farm 在 clone 上复用旧 Human `engine.craft()` 完成重复数量校验、库存扣除、隐藏配方／权威随机、种子发放与任务事件，成功一次保存并返回真实 receipt 与新 catalog，React 只显示权威作物名和等级 | 等待两条发布线提交／部署与真实账号验收；不改变素材、配方、概率、产物或经济规则 |
| 叮咚播报 | React 将任务／成熟／留言／牧场通知按固定顺序放在同一可滚动列表，无分类键、分页或足迹。Live 通过独立 `/api/farm/bulletin` 读取真实持久化来源：成熟地块复用 field 纯投影，任务只读取已接受且未完成的存档槽，未知或不一致来源按分区 unavailable 返回；读取不推进时间、不领取、不刷新、不随机、不保存 | 等待两条发布线提交／部署与真实账号验收；不含足迹、不唤醒普通消息 |
| 设置／探险／图鉴 | `/api/farm/catalog` 为三者提供真实投影。农场名、小机昵称、你的昵称、欢迎语、佩戴／卸下称号及四个社交开关均逐字段走 farm 权威保存；探险沿用既有进入／探索／摇骰／选择／祈福／撤退权威动作；作物图鉴以专用 revision 收藏／取消收藏并显示接口实际发现与详情 | 只按实际字段和既有动作继续扩展；不转发旧表单、解析 HTML 或直写存档 |
| 素材台账 | 类型化 manifest 继续以稳定语义 key／内容 ID 接线场景、工具与图集；受控 source map 和构建覆盖检查核对 60 个声明与当前运行引用，构建生成 43 张内容哈希图片。明确 `missing／fallback` 保留，状态不保存 URL、sprite index 或文件序号，`approved-source` 不进入构建输出 | 177 种作物、55 种鱼及其他尚无逐项正式图的内容继续诚实缺图／fallback，不能把类别图冒充完成 |
| PWA／静态资源 | 社区已有唯一 Manifest 与 Service Worker；导航 NetworkFirst、哈希静态资源 CacheFirst、所有 `/api/**` NetworkOnly且无 Background Sync。四场景、播报、商店、通用工具、行动面板与料理库存均为 lazy chunk，首页不预载行动面板或料理库存；当前构建 170 modules，只保留既存主包大于 500 kB 提醒 | 等待真实安全来源下的冷／热启动、离线边界与更新验收；不新增农场专属 Service Worker，不把本地构建写成已部署 |
...
6. `kitchen` 读取、库存动作、商店刷新／食材／食谱／三件工具购买及 raw item 权威烹饪已经接通；下一步只迁移 90 道食谱的料理方式与工具要求，再开放与权威食谱绑定的一键制作；
7. 按普通市场真实复合定位接一份共享市场，再接探险、设置及已经批准的熔炼读取／动作；
8. 接邻里排行榜、留言板和原创作物，不等待足迹；
9. 农场／牧场购物车已经按一车一次后台幂等快照与一次 Bell 通知启用“喊 TA 来买”，页面不跟踪购买结果；料理台食材、每日食谱与三件付费工具继续由 Human 整车直接购买；
