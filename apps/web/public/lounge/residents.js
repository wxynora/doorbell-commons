...
export function createResidents({scene,draw,effects}){
 const visible=new Map(),desired=new Map();let disposed=false;
 function remove(id){const mesh=visible.get(id);if(!mesh)return false;effects?.remove(id);scene.remove(mesh);mesh.geometry.dispose();mesh.material.map?.dispose();mesh.material.dispose();visible.delete(id);return true;}
 async function loadResident(person,entry){
   ...
   if(visible.get(person.residentId)?.userData.key===key){effects?.set(person.residentId,visible.get(person.residentId),entry.person);return;}
   ...
   mesh.userData={key,residentId:person.residentId,name:person.name};remove(person.residentId);visible.set(person.residentId,mesh);scene.add(mesh);effects?.set(person.residentId,mesh,entry.person);
   draw();
 }
 async function update(presence){
  ...
  await Promise.all(presence.map(person=>{
   ...
   if(!entry||entry.key!==key){entry={key,promise:null,person};desired.set(person.residentId,entry);}
   entry.person=person;
   const mesh=visible.get(person.residentId);if(mesh)effects?.set(person.residentId,mesh,person);
