...
// These are the owner's edits already present in the frozen 60058 page.
// They are a display projection for this edition, not changes to the stored
// reporter manuscript or a new format reporters must submit.
export function presentDailyIssue(issue: LingyeDailyIssue | null): LingyeDailyIssue | null {
  if (!issue || issue.issueDate !== "2026-09-02") return issue;
  const shown: LingyeDailyIssue = { ...issue };
