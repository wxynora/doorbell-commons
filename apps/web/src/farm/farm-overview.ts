...
export function farmLandUpgradeIssueMessage(issue: FarmLandUpgradeIssue): string {
  if (issue.code === "land_upgrade_rejected" && issue.serverMessage) {
    return issue.serverMessage;
  }
  const messages: Readonly<Partial<Record<FarmLandUpgradeIssue["code"], string>>> = {
    state_conflict: "农场状态已经变化，请重新读取后再升级。",
    idempotency_conflict: "这次土地升级请求已经失效，请重新读取后再试。",
    invalid_request: "土地升级请求不符合当前合同。",
    authentication_required: "登录状态已经失效，请返回社区重新登录。",
    qq_not_group_member: "当前 QQ 已不具备社区访问资格。",
    onebot_unavailable: "QQ 群资格核验服务暂时不可用。",
    registration_profile_required: "当前账号还没有完整的居民、家园和农场绑定。",
    farm_not_found: "当前绑定的农场已经不存在。",
    farm_credential_invalid: "当前账号保存的农场凭据已经失效，请重新确认农场绑定。",
    farm_unavailable: "农场暂时没有确认这次升级，可以用同一次请求重试。",
    upstream_contract_unavailable: "农场返回了当前页面无法确认的土地升级结果。",
    network_unavailable: "连接中断，暂时无法确认这次土地升级结果。",
    unexpected_response: "农场返回了当前页面无法识别的土地升级结果。",
  };
  return messages[issue.code] ?? issue.serverMessage ?? "这次土地升级暂时没有完成。";
}
