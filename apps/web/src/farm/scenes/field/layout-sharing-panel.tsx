...
export function LayoutSharingPanel({ onPreview, request = requestLayoutShare }: {
  ...
  return <fieldset className="farm-settings__group" aria-label="布局码">
    <legend>农场布局</legend>
    <p>把布置分享给朋友，或使用朋友的布局码。</p>
    <button className="farm-settings__save" type="button" disabled={busy} onClick={() => void run(false)}>生成我的布局码</button>
    {shareCode ? <div className="farm-settings__item"><span>我的布局码</span><div className="farm-settings__control">
      <input ref={codeField} aria-label="我的布局码" value={shareCode} readOnly onFocus={event => event.currentTarget.select()} />
      <button className="farm-settings__save" type="button" onClick={async () => {
    ...
    <div className="farm-settings__item">
      <label htmlFor="farm-layout-code">朋友的布局码</label>
    ...
    <p>缺少或放不下的装饰会留空。先看预览，确认后才替换当前布置。</p>
    {message || busy ? <p role="status">{busy ? "正在读取布局…" : message}</p> : null}
  </fieldset>;
}
