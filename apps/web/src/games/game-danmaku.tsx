...
export function GameDanmakuLayer({roomId,messages,visible,connected}:{roomId:string;messages:readonly SessionChatMessage[];visible:boolean;connected:boolean}){
  ...
  const [entryUnit,setEntryUnit]=useState(1);
  ...
  useLayoutEffect(()=>{
    ...
    const place=()=>{
      frame=0;
      const box=entryRef.current;
      if(!box)return;
      const self=box.getBoundingClientRect();
      const width=self.width||0,height=self.height||0;
      if(!width||!height)return;
      const icon=document.querySelector<HTMLElement>(".game-rules-icon");
      const gap=8;
      let left,top;
      let focus=0;
      if(icon){
        ...
        focus=rect.height?rect.height/34:0;
      }else{
      ...
      const scale=focus||Math.min(window.innerWidth,window.innerHeight)/390;
      const next=Math.max(0.8,Math.min(scale,1.3));
      setEntryUnit(old=>Math.abs(old-next)<0.01?old:next);
      setAnchor(old=>old&&Math.abs(old.left-left)<0.5&&Math.abs(old.top-top)<0.5?old:{left,top});
    };
  ...
  async function send(phraseId:string){
    if(sending.current||remaining||!connected)return;
  ...
  return <>
    {createPortal(<div className="game-danmaku-screen" style={{'--dm-unit':`${unit}px`} as CSSProperties}>
    ...
    {visible&&<div ref={entryRef} className="game-danmaku-entry" style={{'--dm-unit':`${entryUnit}px`,left:anchor?.left??0,top:anchor?.top??0,visibility:anchor?'visible':'hidden'} as CSSProperties}>
      <button className="game-danmaku-toggle" aria-label="发送围观弹幕" aria-expanded={open} onClick={()=>setOpen(!open)}><svg viewBox="0 0 24 24" fill="none" aria-hidden="true"><path d="M3 5h18v12H9l-5 4v-4H3zM7 9h10M7 13h6" stroke="currentColor" strokeWidth="1.8" strokeLinejoin="round" strokeLinecap="round"/></svg><span>弹幕</span></button>
      {open&&<section className="game-danmaku-picker" aria-label="围观弹幕短句"><header><b>发个弹幕</b><button aria-label="关闭弹幕选项" onClick={()=>setOpen(false)}>×</button></header>
      {connected?<><small>{remaining?`${remaining} 秒后可以再发`:'每分钟可以发一次'}</small>
      <div>{options?.phrases.map(p=><button key={p.id} disabled={busy||remaining>0} onClick={()=>void send(p.id)}>{p.text}</button>)??<p>正在加载…</p>}</div></>
      :<p className="game-danmaku-hint">正在连接围观频道，连上就能发弹幕…</p>}{error&&<p role="alert">{error}</p>}</section>}
    </div>}
  </>;
}
