...
import { buyMidAutumnSeed, buyAllMidAutumnSeeds } from "../../domain/field/mid-autumn-shop.js";
...
export function handleCommerceAction(action, f, b, now) {
    switch (action) {
        ...
        case "buy-seed": { // 买自己店当前刷出的限定种子（金币结算，每种每天限购 1；不填 id 默认买当前刷出的那颗）
            if (b.allin === true) {
                const all = buyAllMidAutumnSeeds(f, now);
                if (all.handled) {
                    const details = all.ok ? all.items.map(item => `${item.name}×${item.qty}`).join("、") : "";
                    return { ok: all.ok, text: all.ok ? withFooter(f, now, `🛒 中秋限定种子已全部买满：${details}，共花费 ${all.cost} 金。`) : all.error };
                }
            }
