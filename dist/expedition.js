...
function dropLabel(d) {
    ...
    if (d.t === "potion")
        return `+加速药水×${d.n}`;
    if (d.t === "decor")
        return `🏡${expDecorById.get(d.id)?.name ?? "装饰"}`;
    return "";
}
function addDrops(exp, drops) {
    if (!drops?.length)
        return [];
...
function weightedSampleNoRepeat(rng, pool, k) {
    const picked = [];
...
const LAYER_RANK = { shallow: 0, deep: 1, finale: 2 };
// 抽 budget 段际遇：未解锁优先（重复进同一秘境时尽量给新格子），不够再用已解锁补；按层排序，浅→深→终景压轴。
function buildQueue(rng, mapId, budget, seen) {
    const map = expMapById.get(mapId);
    const all = map.events.map(ev).filter((e) => !!e);
...
// —— 见闻录：遇到即解锁一格 ——
function recordSeen(f, id) {
    f.expCodex ??= [];
    if (!f.expCodex.includes(id))
        f.expCodex.push(id);
}
...
function advance(f, now) {
    const exp = f.expedition;
    // 跳过坏 id
    while (exp.queue.length && !ev(exp.queue[0]))
        exp.queue.shift();
    if (!exp.queue.length)
        return settle(f, now, "走到了头");
...
export function expChoose(f, optKey, now) {
    ...
    if (!opt)
        return { ok: false, text: `没有选项「${optKey}」。${optionsText(e)}` };
    ...
    const fight = opt.outcomes.find((o) => o.t === "combat");
    if (fight) {
    ...
    if (jump && ev(jump.to))
        exp.queue.unshift(jump.to); // 目标 id 有效才跳；无效则忽略（不崩）
...
function applyOutcomes(f, exp, outcomes) {
    ...
    for (const o of outcomes) {
        ...
        switch (o.t) {
            ...
            case "silver":
            case "potion": {
                addDrops(exp, [{ t: o.t, n: o.n }]);
                lines.push(`（${dropLabel({ t: o.t, n: o.n })} 进行囊）`);
                break;
            }
            case "decor": {
                addDrops(exp, [{ t: "decor", id: o.id }]);
                lines.push(`（${dropLabel({ t: "decor", id: o.id })} 进行囊）`);
                break;
            }
            case "status": {
                exp.hp = Math.max(0, exp.hp + o.n);
                lines.push(o.n >= 0 ? `（❤状态+${o.n}）` : `（❤状态${o.n}）`);
                break;
            }
            case "buff": {
                exp.buffMod += o.mod;
                lines.push(`（下次检定 +${o.mod}）`);
                break;
            }
            case "none":
            default: break;
...
export function expRoll(f, byHuman, now) {
    const exp = f.expedition;
    if (!exp)
    ...
    if (natural === 12 || total >= target + 3)
        ...
        band = "win";
    else if (natural === 2 || total <= target - 3)
        band = "loss";
    else
        band = "graze";
    ...
    if (band === "crit" || band === "win") {
        if (contentEv?.record)
            recordSeen(f, contentEv.id);
        const drops = (c.win?.drops ?? []).slice();
        if (band === "crit" && c.win?.critDrops)
            drops.push(...c.win.critDrops);
        const got = addDrops(exp, drops);
        lines.push(band === "crit" ? "🌟 大胜！" : "✅ 胜！");
        if (c.win?.text)
            lines.push(c.win.text);
        if (got.length)
            lines.push(`（战利品：${got.join("、")} 进行囊）`);
        if (band === "crit" && c.win?.critDrops)
            lines.push("（大胜额外犒赏！）");
        // 默契度：每赢一场 +1，封顶 100
    ...
    exp.pending = null;
    if (!inline)
        exp.queue.shift(); // 内容战斗才消费 queue；内联战斗没有对应事件，别误删下一格
...
function settle(f, now, how) {
    ...
    for (const d of exp.bag) {
        if (d.t === "coins") {
            ...
            f.items.speed_potion = (f.items.speed_potion ?? 0) + (d.n ?? 0);
            potion += d.n ?? 0;
        }
        else if (d.t === "decor") {
...
export function expEnter(f, now, charges = 1) {
    ...
    if (remaining <= 0)
        return { ok: false, text: `🗺️ 今天的探险次数用完了（${EXP_DAILY_CAP}/${EXP_DAILY_CAP}），明天再来。` };
    ...
    if (!open.length)
        return { ok: false, text: "暂时没有可去的秘境。" };
    const rng = new Rng(f.rngState);
...
export function expExplore(f, now, charges = 1) {
    if (!f.expedition)
        return expEnter(f, now, charges);
    const exp = f.expedition;
    if (exp.pending?.type === "choice")
...
// —— retreat：主动撤退，行囊落袋 ——
export function expRetreat(f, now) {
    if (!f.expedition)
        return { ok: false, text: "你现在没在探险。" };
...
export function expSetCharm(f, kind, blessing, now) {
    ...
    if (f.expedition) {
        ...
        if (bl) {
            cur.blessing = bl;
            f.expedition.charmEchoed = false;
        }
