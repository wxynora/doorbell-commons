...
function togetherHistoryHtml(history) {
    return history.map((entry) => {
        ...
        if (entry.kind === "task")
            return `<div style="padding:10px 0;border-top:1px dashed var(--line)"><b>公共任务【${esc(entry.title)}】${entry.contributions?.length ?? 0}/${entry.target ?? 3}</b><div class="small" style="white-space:pre-wrap;margin-top:4px;line-height:1.75">${esc(entry.text)}</div>${entry.contributions?.length ? `<div class="small muted" style="margin-top:8px">${entry.contributions.map((item) => `· ${esc(item.farmName)}：${esc(item.fact)}`).join("<br>")}</div>` : ""}</div>`;
        if (entry.kind === "clue")
...
export function uiTogether(f, publicWorld, now, key) {
    ...
    const progressArt = shared.artFile ? `<figure style="margin:12px 0 14px;border-radius:16px;overflow:hidden;border:1px solid #d5dfc3;background:#e8eee0;box-shadow:0 8px 20px rgba(63,83,45,.10)"><img src="${BASE}/assets/lingye-together/${esc(shared.artFile)}?v=20260815a" alt="" aria-hidden="true" width="1448" height="1086" decoding="async" fetchpriority="high" style="display:block;width:100%;height:auto;aspect-ratio:4/3;object-fit:cover"></figure>` : "";
    ...
    const currentStory = shared.currentTask ? `<div style="margin-top:12px;padding:12px;border-radius:14px;background:#f4f7ea;border:1px solid #d9e3c6">
      ...
      <b>公共任务【${esc(shared.currentTask.name)}】${shared.currentTask.progress}/${shared.currentTask.target}</b>
      <div class="small" style="white-space:pre-wrap;margin-top:6px;line-height:1.7">${esc(shared.currentTask.opening)}</div>
      ${shared.currentTask.options ? `<div class="small" style="margin-top:8px;line-height:1.75">${Object.entries(shared.currentTask.options).map(([option, label]) => `${esc(option)}．${esc(label)}`).join("<br>")}</div>` : ""}
    ...
    const choice = shared.currentChoice ? `<div style="margin-top:12px;padding:12px;border-radius:14px;background:#fff9e9;border:1px solid #ead9ad">
      <b>${shared.currentChoice.index ? `${shared.currentChoice.index}/${shared.currentChoice.total} · ` : ""}${esc(shared.currentChoice.title)}</b>
    ...
    const clueBook = shared.clues.length ? `<details style="margin-top:12px"><summary style="cursor:pointer;font-weight:700">🔎 铃野共行·线索册 ${shared.clues.length}/${shared.clueTarget}</summary>
