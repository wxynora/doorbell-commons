...
const SCENE_URLS = {
    objects: "/assets/qixi-2026/objects-discovery-bg-v3.jpg",
    return: "/assets/qixi-2026/objects-return-bg-v3.jpg",
    lanternNight: "/assets/qixi-2026/lantern-night-bg-v4.jpg",
};
...
.qixi-page .btn.qixi-plaque-button{display:flex;align-items:center;justify-content:center;justify-self:center;width:min(100%,var(--qixi-plaque-width));height:var(--qixi-plaque-height);min-height:0;padding:0 28px;border:0;border-radius:0;background:url('/assets/qixi-2026/qixi-action-plaque-v1.png?v=1') center/100% 100% no-repeat;color:#ffe4a6;box-shadow:none;text-shadow:0 2px 3px rgba(60,35,83,.78);font:800 var(--qixi-plaque-font)/1.2 var(--serif);letter-spacing:.03em}.qixi-page .btn.qixi-plaque-button:active{transform:translateY(1px);box-shadow:none}
...
export function uiQixiLantern(farm, world, now, key, flash, showLetter = false, selectedObjectId) {
    ...
    const body = `${QIXI_STYLE}<main class="qixi-page qixi-stage-${taskView.stage} qixi-time-${isNight ? "night" : "day"}" style="--qixi-scene:url('${scene}')" data-qixi-initial="${showLetter || taskView.finalStageOpen ? "home" : hasSelectedObject ? "lost" : "home"}" aria-label="灯河有信">
      <section class="qixi-home" data-qixi-home>
        ...
        <img class="qixi-scene-title" src="/assets/qixi-2026/qixi-scene-title-v1.png?v=1" alt="" aria-hidden="true">
        ${decorShortcut}
        ${riverActionPanel}
