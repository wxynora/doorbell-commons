...
.qixi-page .btn.qixi-plaque-button{display:flex;align-items:center;justify-content:center;justify-self:center;width:min(100%,var(--qixi-plaque-width));height:var(--qixi-plaque-height);min-height:0;padding:0 28px;border:0;border-radius:0;background:url('${BASE}/assets/qixi-2026/qixi-action-plaque-v1-web.webp?v=1') center/100% 100% no-repeat;color:#ffe4a6;box-shadow:none;text-shadow:0 2px 3px rgba(60,35,83,.78);font:800 var(--qixi-plaque-font)/1.2 var(--serif);letter-spacing:.03em}.qixi-page .btn.qixi-plaque-button:active{transform:translateY(1px);box-shadow:none}
...
.qixi-reward-overlay{position:absolute;z-index:22;inset:0;display:grid;place-items:center;padding:18px;background:rgba(25,34,65,.58)}.qixi-reward-overlay[hidden]{display:none}.qixi-reward-panel{position:relative;width:min(100%,390px);padding:20px 20px 18px;border:1px solid rgba(224,182,119,.9);border-radius:24px;background:radial-gradient(circle at 50% 0,rgba(255,255,255,.94),rgba(255,244,226,.97) 62%,rgba(250,230,216,.97));box-shadow:0 20px 60px rgba(23,31,56,.28),inset 0 0 0 3px rgba(255,255,255,.54);text-align:center}.qixi-reward-close{position:absolute;z-index:3;top:8px;right:10px;width:34px;height:34px;padding:0;border:0;background:transparent;color:#8e6978;font:300 29px/1 Georgia,serif;cursor:pointer}.qixi-reward-lamp{height:112px;margin:-13px auto 0}.qixi-reward-lamp .qixi-lamp-preview{transform:scale(.62);transform-origin:center top}.qixi-reward-kicker{margin:0;color:#ad7477;font:800 9px/1.3 var(--serif);letter-spacing:.16em}.qixi-reward-panel h2{margin:5px 0 0;color:#765466;font:800 21px/1.3 var(--serif)}.qixi-reward-purse{display:flex;align-items:baseline;justify-content:center;gap:12px;margin:10px 0 12px;color:#806370;font-size:11px}.qixi-reward-purse b{margin-right:3px;color:#aa6d79;font:800 19px/1 var(--serif)}.qixi-reward-purse i{width:3px;height:3px;border-radius:50%;background:#d2a46f}.qixi-reward-keepsakes{margin:0 5px 14px;border-top:1px solid rgba(205,163,103,.42);border-bottom:1px solid rgba(205,163,103,.42)}.qixi-reward-keepsakes p{display:grid;grid-template-columns:64px 1fr;align-items:baseline;gap:8px;margin:0;padding:9px 2px;color:#8b747d;font-size:10px;text-align:left}.qixi-reward-keepsakes p+p{border-top:1px dashed rgba(205,163,103,.32)}.qixi-reward-keepsakes strong{color:#765466;font:800 14px/1.3 var(--serif)}.qixi-reward-panel .qixi-plaque-button{margin:0 auto}
...
for(const button of root.querySelectorAll('[data-qixi-reward-close]'))button.addEventListener('click',async()=>{const overlay=button.closest('[data-qixi-reward-overlay]');overlay.hidden=true;const next=new URL(location.href);next.searchParams.delete('flash');history.replaceState(null,'',next);try{await fetch(overlay.dataset.qixiRewardAck,{method:'POST',credentials:'same-origin',headers:{'content-type':'application/x-www-form-urlencoded'},body:'seen=1'})}catch{}});
...
for(const textarea of root.querySelectorAll('[data-qixi-letter-input]')){const fit=()=>{textarea.style.paddingTop='0px';textarea.style.paddingBottom='0px';textarea.style.height='1px';const contentHeight=Math.min(85,Math.max(24,textarea.scrollHeight));const pad=Math.max(0,(85-contentHeight)/2);textarea.style.height='85px';textarea.style.paddingTop=pad+'px';textarea.style.paddingBottom=pad+'px'};textarea.addEventListener('input',fit);fit()}
...
function sideCard(side, state, aiName) {
    ...
    const receivedText = side === "human" && received?.deliveredAt ? displayLetter(received, `${aiName}写来的灯笺`, aiName) : "";
...
function latestCaughtLetter(state, aiName) {
    const partner = state?.lamps?.ai?.deliveredAt ? { ...state.lamps.ai, authorName: aiName, heading: `${aiName}写来的灯笺`, isPartner: true } : null;
...
function letterOverlay(state, showLetter, aiName) {
    if (!showLetter)
        return "";
    const letter = latestCaughtLetter(state, aiName);
    if (!letter)
        return "";
    const note = letter.isPartner === true ? "属于这一户的灯已经抵达。" : "这是一盏经过此处的灯；属于你的那盏还在水路上。";
...
function rewardOverlay(state, key) {
    if (!state?.rewardedAt || state.rewardNoticeSeenAt)
        return "";
    const lamp = state.lamps?.human ?? state.lamps?.ai;
    if (!lamp)
        return "";
    const reward = qixiLantern2026.reward;
    return `<section class="qixi-reward-overlay" data-qixi-reward-overlay data-qixi-reward-ack="${BASE}/ui/${key}/qixi/reward-seen" role="dialog" aria-modal="true" aria-labelledby="qixi-reward-title"><div class="qixi-reward-panel"><button class="qixi-reward-close" type="button" data-qixi-reward-close aria-label="关闭奖励">×</button><div class="qixi-reward-lamp">${lanternPreview(lamp.appearance)}</div><p class="qixi-reward-kicker">首次放灯奖励已送达</p><h2 id="qixi-reward-title">灯河收下了你的灯</h2><div class="qixi-reward-purse"><span><b>${Number(reward.coins) || 0}</b>金币</span><i></i><span><b>${Number(reward.silver) || 0}</b>银币</span></div><div class="qixi-reward-keepsakes"><p><span>限定称号</span><strong>${esc(reward.titleName)}</strong></p><p><span>限定成就</span><strong>${esc(reward.achievementName)}</strong></p></div><button class="btn qixi-plaque-button" type="button" data-qixi-reward-close>收好这份纪念</button></div></section>`;
}
...
function lanternPreview(appearance, form = false) {
    ...
    const patternLayer = saved.pattern === "none" && !form ? "" : `<span class="qixi-lantern-layer qixi-lantern-pattern" data-lamp-pattern${saved.pattern === "none" ? " hidden" : ""} style="${box(patternLayout)};background-position:${pattern[0]} ${pattern[1]}"></span>`;
    const ornamentLayer = saved.ornament === "none" && !form ? "" : `<span class="qixi-lantern-layer qixi-lantern-ornament" data-lamp-ornament${saved.ornament === "none" ? " hidden" : ""} style="${box(ornamentLayout)};background-position:${ornament[0]} ${ornament[1]}"></span>`;
    const sealLayer = saved.seal === "none" && !form ? "" : `<span class="qixi-lantern-layer qixi-lantern-seal" data-lamp-seal${saved.seal === "none" ? " hidden" : ""} style="${box(sealLayout)};background-position:${seal[0]} ${seal[1]}"></span>`;
...
export function uiQixiLantern(farm, world, now, key, flash, showLetter = false, selectedObjectId) {
    const aiName = String(farm.aiName ?? "").trim() || "小机";
    ...
    const receivedLetterOverlay = letterOverlay(state, showLetter, aiName);
    const releaseRewardOverlay = rewardOverlay(state, key);
