...
function assertRuntime(runtime) {
  if (!runtime || typeof runtime.read !== "function" || typeof runtime.readReceipt !== "function" || typeof runtime.draw !== "function" || typeof runtime.drawTen !== "function") {
...
export function createDoorbellGachaRuntime(options = {}) {
  ...
  return Object.freeze({
    ...
    readReceipt: (input) => service.readReceipt(input),
    draw: (input) => service.draw(input),
    drawTen: (input) => service.drawTen(input),
  });
}

export async function handleDoorbellHumanGachaReceiptRead(req, res, method, runtime) {
  ...
  try {
    ...
    return jsonOut(res, 200, assertRuntime(runtime).readReceipt(parsed.input));
  } catch (error) {
