...
function nav(key, active) {
    const items = [
        ["", "🏡 主页"], ["ranch", "🐮 我的牧场"], ["glimmer", "✨ 流光原野"], ["together", "🧭 铃野共行"], ["cooking", "🍳 料理台"], ["ta", "✍️ TA的农场"], ["expedition", "🗺️ 探险"], ["codex", "📖 图鉴册"], ["messages", "📮 留言板"], ["leaderboard", "🏆 排行榜"],
    ];
...
function togetherHistoryHtml(history) {
    return history.map((entry) => {
        if (entry.kind === "choice")
            return `<div style="padding:8px 0;border-top:1px dashed var(--line)"><b>选择 ${entry.step} · ${esc(entry.option)}</b><br><span class="small muted">${esc(entry.farmName)}选择：${esc(entry.label)}</span></div>`;
        if (entry.kind === "task")
...
export function uiTogether(f, publicWorld, now, key) {
    ...
    const choice = shared.currentChoice ? `<div style="margin-top:12px;padding:12px;border-radius:14px;background:#fff9e9;border:1px solid #ead9ad">
      ...
      <div class="small" style="margin-top:7px;line-height:1.75">${Object.entries(shared.currentChoice.options ?? {}).map(([option, label]) => `${esc(option)}．${esc(label)}`).join("<br>")}</div>
    </div>` : "";
    ...
    const body = `<div class="plaque"><h1>🧭 铃野共行</h1><p class="welcome">全服共同推进的公共故事；这里不会混入个人探险进度。</p>
      ...
      <section class="card" style="border:2px solid #a9bd83;background:linear-gradient(180deg,#fbfff5,#f7f3e7)">
        ...
        ${progressArt}${currentStory}${choice}${ownReward ? `<div class="flash" style="white-space:pre-wrap;margin-top:12px">${esc(publicExpeditionRewardText(ownReward))}</div>` : ""}
