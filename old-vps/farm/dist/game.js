...
import { advance, plant, water, harvest, upgradeLand, shopOffer, collectionPct, codexCountByCategory, nextUpgradeReq, buyItem, useItem, craft, designCrop, plantBatch, waterAll, harvestAll, usePotionBatch, refreshShop, buyRecipe, affordablePotions, buyPotionSet, potionDailyLeft, shopAnimals, nextLockedAnimal, buyAnimalForPartner, ranchRoamLine, decorLines, takeInbox, shopPets, nextLockedPet, buyPetForPartner, farmSendRanch, potionTargets, circledNum, isUgcCrop, } from "./engine.js";
...
import { landTierByLevel, crops, cropById, getCrop, animalById, petById, totalCropCount, cropsByCategory, materials, materialById, recipes, } from "./content.js";
import { ITEMS, STARTING_COINS, STARTER_POTIONS, MATERIAL_REF_PRICE, MARKET_FEE, REPORT_THRESHOLD, UGC_VALUE, LIMITED_SEED_REF_DISCOUNT, POTION_DAILY_CAP, POTION_CAP_LINE, MESSAGES_MAX, WELCOME_MAX, UGC_NAME_MAX, SEED_PRICE, GROW_TICKS, NPC_ID, NPC_NAME } from "./config.js";
...
function composeHarvests(rs, compact, harvesterId) {
    if (!compact) {
...
export function viewShop(f, now) {
    ...
    if (f.shop.recipe) {
        ...
        if (out)
            recipeLine = `📜 配方上架：一张能熔出【${out.name}·${out.rarity}】的配方（具体素材组合，买下才揭晓）　500金 → buy-recipe`;
    }
...
export function ranchShopSection(f) {
    ...
    if (!avail.length) {
        ...
        for (const a of avail)
            lines.push(`· ${a.emoji ? a.emoji + " " : ""}${a.name}（${a.buyCost}金）产${a.produce}　→ buy-animal {"id":"${a.id}"}`);
        const nx = nextLockedAnimal(f);
        if (nx)
    ...
    if (availPets.length) {
        ...
        for (const p of availPets)
            lines.push(`· ${p.emoji} ${p.name}（${p.buyCost}金）${p.tag}　→ buy-pet {"id":"${p.id}"}`);
    }
    else {
...
export function viewLedger(f) {
    ...
    const rows = log.slice(0, 12).map((e) => {
        const icon = (e.type === "buy-animal" || e.type === "buy-pet") ? "🐾 -" : e.type === "remit" ? "💰 +" : e.type === "send-ranch" ? "💰 -" : "🧪 +";
...
export function buyFromMarket(seller, buyer, kind, id, qty) {
    ...
    return { ok: true, name: itemName(kind, id), qty: n, cost, fee, price: e.price };
}
...
function plantHint(f, cropId, cropName) {
