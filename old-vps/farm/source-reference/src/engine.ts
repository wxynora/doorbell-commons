...
import {
  TICK_MS, GROW_TICKS, SEED_PRICE, WATER_LUCK_PER, WATER_LUCK_CAP, MAX_LOG, TRAIL_MAX, LAND_UPGRADE_REQ,
...
import {
  crops, cropById, getCrop, animals, animalById, pets, petById, accessories, decorations, accessoryById, decorationById, landTierByLevel, landTiers, totalCropCount, festivals,
  qualities, materials, materialById, recipes, specialEvents, expDecorById,
  type Crop, type Quality, type Material, type BonusEvent, type Animal, type Pet,
} from "./content.js";
...
import type { Farm, Plot, CodexEntry, Ranch, RanchRaid, RanchRaidHistoryEntry, AnimalInst, PetInst } from "./types.js";

/** 取（必要时补发）人类前端钥匙。老农场没有就现生成一把；调用方负责 save()。 */
export function ensureHumanKey(farm: Farm): string {
  if (!farm.humanKey) farm.humanKey = randomUUID().replace(/-/g, "");
  return farm.humanKey;
}

export function pushLog(farm: Farm, msg: string): void {
  farm.log.push(msg);
  if (farm.log.length > MAX_LOG) farm.log.splice(0, farm.log.length - MAX_LOG);
}
/** 记一条足迹（别人对本农场的社交动作：帮浇水 / 偷菜得手 / 被狗吓退）；最新在前，超上限截尾。 */
export function pushTrail(farm: Farm, ev: NonNullable<Farm["trail"]>[number]): void {
  (farm.trail ??= []).unshift(ev);
  if (farm.trail.length > TRAIL_MAX) farm.trail.length = TRAIL_MAX;
}

type Fail = { ok: false; error: string };
type Ok<T> = { ok: true } & T;

const humanDisplay = (farm: Farm) => farm.humanName || "伴侣";
const aiDisplay = (farm: Farm) => farm.aiName || farm.name || "对方";

// —— 惰性结算（纯时间生长，无缺水停滞）——
export function advance(farm: Farm, now: number): number {
  ...
  for (const p of farm.plots) {
    if (p.crop && !p.crop.ripe) {
      ...
      if (p.crop.progress >= p.crop.growTicks) p.crop.ripe = true;
    }
  }
  // 牧场动物随时间攒产出（伴侣在人类前端收）；等级越高每周期产得越多
  if (farm.ranch) for (const a of farm.ranch.animals) {
    const kind = animalById.get(a.kindId);
    if (!kind) continue;
    // 不累加：只在「没有未收产出」时推进生产，攒到 1 份就停，伴侣收了才产下一份（挂机不堆积）
    if (a.pending < 1) {
      a.ticksSinceProduce += elapsed;
      if (a.ticksSinceProduce >= kind.produceEveryTicks) { a.pending = 1; a.ticksSinceProduce = 0; }
    }
  }
...
// —— 图鉴 ——
function addCodex(farm: Farm, cropId: string, qualityTier: number, now: number): boolean {
  const prev = farm.codex[cropId] as CodexEntry | undefined;
  if (!prev) {
...
export function officialCodexCount(farm: Farm): number {
  return Object.keys(farm.codex).filter((id) => cropById.has(id)).length;
}
export function collectionPct(farm: Farm): number {
  // 只算官方作物（UGC 自创不计入官方收集度）
  return officialCodexCount(farm) / totalCropCount;
}
/** 图鉴星标：切换某作物的收藏态。返回 { ok, on, name }；on=切换后是否已收藏。 */
export function toggleStar(farm: Farm, id: string): { ok: boolean; on: boolean; name?: string } {
  const crop = getCrop(id);
  if (!crop) return { ok: false, on: false };
  farm.starred ??= [];
  const i = farm.starred.indexOf(id);
  if (i >= 0) { farm.starred.splice(i, 1); return { ok: true, on: false, name: crop.name }; }
  farm.starred.push(id);
  return { ok: true, on: true, name: crop.name };
}
/** 某作物是否被伴侣星标收藏。 */
export function isStarred(farm: Farm, id: string): boolean {
  return !!farm.starred?.includes(id);
}
/** 按 农场id+日 的确定性 0~1 值（不消耗 rngState；同一天同一农场恒定）。给夜间商店「有概率刷出」用。 */
function dayHash(id: string, day: number): number {
  let h = (2166136261 ^ day) >>> 0;
  for (let i = 0; i < id.length; i++) { h ^= id.charCodeAt(i); h = Math.imul(h, 16777619); }
  return ((h >>> 0) % 100000) / 100000;
}
/** 已集齐的某类别作物种类数（升级牵制用） */
export function codexCountByCategory(farm: Farm, cat: string): number {
  return Object.keys(farm.codex).filter((id) => cropById.get(id)?.category === cat).length;
}

// —— 限定作物可种判定（公历节日 + 结构化解锁规则 unlockRule；回落老的图鉴%文案）——
export function isLimitedAvailable(crop: Crop, farm: Farm, now: number): boolean {
  if (crop.category !== "limited") return false;
...
function resolveLimitedRef(farm: Farm, ref: string): string | undefined {
  ref = String(ref ?? "").trim();
  if (!ref) return undefined;
  const direct = getCrop(ref); // 已经是 id
  if (direct && (direct.category === "limited" || direct.category === "ugc")) return ref;
  for (const id of Object.keys(farm.seeds)) { // 按中文名找背包里有的（含自己设计/熔炼/买来的）
    const c = getCrop(id);
    if (c && c.name === ref && (c.category === "limited" || c.category === "ugc")) return id;
  }
...
export function plant(
  farm: Farm, plotId: number, seedType: "common" | "fantasy" | "limited", limitedId: string | undefined, now: number,
): Ok<{ seedType: string; limitedId?: string }> | Fail {
  const plot = farm.plots.find((p) => p.id === plotId);
  if (!plot) return { ok: false, error: `没有 ${plotId} 号地` };
  if (plot.crop) return { ok: false, error: `${plotId} 号地已经种着东西了` };
  ...
  if (seedType === "limited") {
    ...
    if (!resolved) return { ok: false, error: "没有这种限定/自创作物（填它的中文名或 id，名字去 bag 抄）" };
    limitedId = resolved;
    ...
    if ((farm.seeds[limitedId!] ?? 0) > 0) {
      ...
      if (farm.seeds[limitedId!] <= 0) delete farm.seeds[limitedId!];
    } else {
    ...
    if (crop.category === "ugc") onTaskEvent(farm, "plant_ugc", now); // 随机任务：种下一株自创作物
  ...
  if (farm.coins < price) return { ok: false, error: `金币不足，${seedType === "common" ? "普通" : "奇幻"}种子要 ${price}` };
  farm.coins -= price;
...
export function water(farm: Farm, plotId: number, by: string, isOwner: boolean): Ok<{ by: string; isOwner: boolean; capped: boolean }> | Fail {
  const plot = farm.plots.find((p) => p.id === plotId);
  if (!plot || !plot.crop) return { ok: false, error: `${plotId} 号地没有作物` };
  const capped = plot.crop.waterCount >= WATER_CAP_COUNT;
  if (!capped) plot.crop.waterCount += 1;
...
const remainingTicks = (c: NonNullable<Plot["crop"]>) => c.growTicks - c.progress;
export function visitorWater(
  farm: Farm, visitorId: string, plotId: number | undefined, by: string, now: number,
): Ok<{ plotId: number; ripened: boolean }> | Fail {
  const day = currentDayIndex(now);
  if (farm.waterVisits?.[visitorId] === day) return { ok: false, error: "已浇过水，明天再来吧。" };
  let plot: Plot | undefined;
  if (plotId != null) {
    plot = farm.plots.find((p) => p.id === plotId);
    if (!plot || !plot.crop) return { ok: false, error: `${plotId} 号地没有作物` };
    if (plot.crop.ripe) return { ok: false, error: `${plotId} 号地的作物已经熟了，浇了也没用` };
  } else {
    const growing = farm.plots.filter((p) => p.crop && !p.crop.ripe);
    if (!growing.length) return { ok: false, error: "对方没有可浇水的作物" };
...
// —— 揭晓 roll（收获/偷菜共用）——
function reveal(farm: Farm, plot: Plot, now: number): Ok<{ crop: Crop; quality: Quality; value: number }> | Fail {
  const rng = new Rng(farm.rngState);
  const c = plot.crop!;
  let crop: Crop;
  if (c.seedType === "limited") {
    const planted = getCrop(c.limitedId!);
    if (!planted) return { ok: false, error: "这块地里的作物数据已失效，无法揭晓" };
    crop = planted;
  }
  else crop = rollCrop(rng, c.seedType, farm.landTier, c.waterCount, currentSeason(now).name);
...
// —— 收获奖励事件 + 特殊掉落 ——
export type BonusResult = { name: string; text: string; effectType: string; extraCoins: number; ripened: number } | null;
export type DropResult = { id: string; name: string; rarity: string; desc: string } | null;

function qualityTierFromName(name: string): number {
  if (name.includes("优")) return 3; // "优品" ≈ 良品
  return qualities.find((q) => q.name === name)?.tier ?? 3;
}
function pickMaterial(rng: Rng): Material {
  return materials[rng.weighted(materials.map((m) => MATERIAL_DROP_WEIGHT[m.rarity] ?? 1))];
}
function ripenAdjacent(farm: Farm, exclude: Plot, n: number): number {
  let c = 0;
  for (const p of farm.plots) {
    if (c >= n) break;
    if (p === exclude || !p.crop || p.crop.ripe) continue;
    p.crop.ripe = true;
...
function rollBonusEvent(rng: Rng): BonusEvent | null {
  if (rng.next() >= HARVEST_EVENT_CHANCE) return null;
  const ev = specialEvents.bonus;
  return ev.length ? ev[rng.weighted(ev.map((e) => e.weight))] : null;
}

// 季节收获事件:这批是否还能吃到效果(带计数上限的如知时雨/蜂媒最多 6 株)
const seasonApplies = (mod?: SeasonHarvestMod | null): mod is SeasonHarvestMod => !!mod && (!mod.capLeft || mod.capLeft.n > 0);
const qualityByTier = (t: number) => qualities.find((q) => q.tier === t);

// —— 收获 ——
// seasonMod：季节「收获型」事件的本批修正（rollSeasonHarvest 给的）；非空即触发了季节事件 → 抑制原有收获奖励事件（互斥）。
export function harvest(
  farm: Farm, plotId: number, now: number, seasonMod?: SeasonHarvestMod | null,
): Ok<{ crop: Crop; quality: Quality; value: number; isNew: boolean; codexReward: number; bonus: BonusResult; drop: DropResult; potionDrop: boolean }> | Fail {
  const plot = farm.plots.find((p) => p.id === plotId);
  if (!plot || !plot.crop) return { ok: false, error: `${plotId} 号地没有作物` };
  if (!plot.crop.ripe) return { ok: false, error: "作物还没成熟" };
  ...
  if (apply) {
    if (seasonMod!.type === "quality_top") quality = qualityByTier(4) ?? quality;          // 极品
    else if (seasonMod!.type === "quality_min") quality = qualityByTier(1) ?? quality;     // 袖珍
    else if (seasonMod!.type === "quality_down") quality = qualityByTier(Math.max(1, quality.tier - (seasonMod!.value ?? 1))) ?? quality;
  }
  ...
  if (ev) {
    if (ev.effectType === "品相保底") {
      ...
      if (quality.tier < t) quality = qualities.find((q) => q.tier === t) ?? quality;
    }
  ...
  if (ev?.effectType === "倍率") value = Math.round(value * (Number(ev.param) || 1));
  if (apply && seasonMod!.type === "value_mult") value = Math.round(value * (seasonMod!.value ?? 1)); // 知时雨/雪被：本批价值×2
  if (apply && seasonMod!.capLeft) seasonMod!.capLeft.n -= 1; // 吃掉一株名额
  farm.coins += value;
  if (bonus?.extraCoins) farm.coins += bonus.extraCoins;

  // 素材掉落（攒来熔炼限定种子；招财猫把概率温和拉高）
  let drop: DropResult = null;
  ...
  if (potionDrop) farm.items.speed_potion = (farm.items.speed_potion ?? 0) + 1;
  farm.rngState = rng.state;
  ...
  if (codexReward) farm.coins += codexReward;
...
export function craft(farm: Farm, materialIds: string[], _now: number): Ok<{ cropId: string; cropName: string; rarity: string; byRecipe: boolean }> | Fail {
  ...
  for (const x of materialIds) {
    ...
    if (!m) return { ok: false, error: `没有这种素材: ${x}` };
    ids.push(m.id);
  }
  // 校验库存（含重复计数）
  ...
  for (const [id, n] of Object.entries(need)) {
    if ((farm.materials[id] ?? 0) < n) return { ok: false, error: `素材不足: ${materialById.get(id)!.name}` };
  }
  // 消耗
  for (const [id, n] of Object.entries(need)) {
    farm.materials[id] -= n;
    if (farm.materials[id] <= 0) delete farm.materials[id];
  }
...
function stealQuota(farm: Farm, now: number): { day: number; n: number; lastAt?: number } {
  const day = currentDayIndex(now);
  if (!farm.stealQuota || farm.stealQuota.day !== day) farm.stealQuota = { day, n: 0 };
  return farm.stealQuota;
}
const fmtStealRemain = (ms: number) => {
  const min = Math.max(1, Math.ceil(ms / 60000));
  return min >= 60 ? `${Math.ceil(min / 60)}小时` : `${min}分钟`;
};
export function stealAvailability(thief: Farm | undefined, now: number): { ok: true; left: number } | { ok: false; reason: string } {
  if (!thief) return { ok: true, left: STEAL_DAILY_CAP };
  const q = stealQuota(thief, now);
  if ((q.n ?? 0) >= STEAL_DAILY_CAP) return { ok: false, reason: `今天已经偷满 ${STEAL_DAILY_CAP} 次了，明天再出门吧。` };
  const leftMs = q.lastAt ? q.lastAt + STEAL_COOLDOWN_MS - now : 0;
  if (leftMs > 0) return { ok: false, reason: `刚偷过菜，先歇一会儿，约 ${fmtStealRemain(leftMs)} 后再来。` };
  return { ok: true, left: STEAL_DAILY_CAP - (q.n ?? 0) };
}
export function canStealNow(thief: Farm | undefined, now: number): boolean {
  return stealAvailability(thief, now).ok;
}
/** 放偷冷却：本农场被偷一次后 30 分钟内谁都偷不了；返回剩余毫秒（0=没保护/已过期）。 */
export function stealShieldRemain(victim: Farm, now: number): number {
  return Math.max(0, (victim.stealShieldUntil ?? 0) - now);
}
/** 这株是不是原创(ugc)作物——原创受保护、禁止偷，只能去集市买种子自己种。 */
export function isUgcCrop(crop: Plot["crop"]): boolean {
  return !!crop && crop.seedType === "limited" && !!crop.limitedId && getCrop(crop.limitedId)?.category === "ugc";
}
function recordStealAttempt(thief: Farm | undefined, now: number): void {
  if (!thief) return;
  const q = stealQuota(thief, now);
  q.n = (q.n ?? 0) + 1;
  q.lastAt = now;
...
export function steal(
  victim: Farm, plotId: number, by: string, now: number, thief?: Farm,
): Ok<{ crop: Crop; quality: Quality; value: number; isNewForThief: boolean; codexReward: number }> | Fail {
  if (victim.id === by) return { ok: false, error: "不能偷自己的菜；收自己地里的作物请用 harvest" };
  ...
  if (shieldMs > 0) return { ok: false, error: `这家刚被偷过还在防备，约 ${fmtStealRemain(shieldMs)} 后才能再下手。` };
  const plot = victim.plots.find((p) => p.id === plotId);
  if (!plot || !plot.crop) return { ok: false, error: `${plotId} 号地没有作物，晚了一步` };
  if (!plot.crop.ripe) return { ok: false, error: "作物还没成熟，偷不了" };
  ...
  if (refund > 0) victim.coins += refund;
  plot.crop = null;
  recordStealAttempt(thief, now);
  victim.stealCooldowns[by] = now;
  if (victim.id !== NPC_ID) victim.stealShieldUntil = now + STEAL_SHIELD_MS; // 放偷冷却：这家 30 分钟内谁都偷不了（阿土是常驻练手靶，豁免）
...
function finishRanchRaidHistory(ranch: Ranch, raid: RanchRaid, status: "caught" | "returned", coins: number): void {
  const history = ranch.raidHistory;
  if (!history || history.day !== currentDayIndex(raid.startedAt)) return;
...
export function dispatchRanchRaid(owner: Farm, target: Farm, animalIdx: number, durationHours: number, now: number): Ok<{ raid: RanchRaid; animal: string }> | Fail {
  ...
  const day = currentDayIndex(now);
  if (!ranch.raidHistory || ranch.raidHistory.day !== day) ranch.raidHistory = { day, entries: [] };
  const historyEntry: RanchRaidHistoryEntry = {
    raidId: raid.id,
    animalName,
    targetFarmId: target.id,
...
export function catchRanchRaid(target: Farm, owners: Farm[], raidId: string, now: number): Ok<{ owner: string; animal: string; compensation: number }> | Fail {
  for (const owner of owners) {
    ...
    if (compensation > 0) {
      const day = currentDayIndex(now);
      if (!owner.ranch!.raidLoss || owner.ranch!.raidLoss.day !== day) owner.ranch!.raidLoss = { day, n: 0 };
    ...
    const caughtText = `🚨 你的${animalName}在「${targetLabel}」被抓住，赔给对方 ${compensation} 金`;
  ...
  for (const owner of farms) {
    ...
    for (const raid of ranch.raids) {
      ...
      if (target && target.id !== owner.id) {
        const day = currentDayIndex(now);
        if (!ranch.raidIncome || ranch.raidIncome.day !== day) ranch.raidIncome = { day, n: 0 };
        ...
        if (stolenCoins > 0) {
          if (!targetRanch.raidLoss || targetRanch.raidLoss.day !== day) targetRanch.raidLoss = { day, n: 0 };
...
function pushLedger(farm: Farm, type: "buy-animal" | "buy-pet" | "remit" | "send-ranch" | "potion", amount: number, note: string, now: number): void {
  (farm.ledger ??= []).unshift({ at: now, type, amount, note });
  if (farm.ledger.length > LEDGER_MAX) farm.ledger.length = LEDGER_MAX;
}

/** AI 买一只已解锁的动物送给伴侣（花 farm.coins，动物进牧场；这是机→人的"金币互传往来"）。 */
export function buyAnimalForPartner(farm: Farm, id: string, now: number): Ok<{ name: string; cost: number }> | Fail {
  const kind = animalById.get(String(id));
  if (!kind) return { ok: false, error: `没有这种动物：${id}（看商店里已解锁的动物）` };
  if (!animalUnlocked(farm, kind)) return { ok: false, error: `${kind.name}还没解锁——${kind.unlockCond ?? `图鉴集齐 ${kind.unlockCodex} 种`}（你现在 ${officialCodexCount(farm)} 种）` };
  if (farm.ranch?.animals.some((a) => a.kindId === kind.id)) return { ok: false, error: `${kind.name}已送养过，不再上架。` };
  if (farm.coins < kind.buyCost) return { ok: false, error: `金币不足，${kind.name}要 ${kind.buyCost} 金（你有 ${farm.coins}）` };
...
export function petUnlocked(farm: Farm, kind: Pet): boolean {
  return officialCodexCount(farm) >= kind.unlockCodex;
}
/** 当前已解锁、会上架商店的宠物。 */
export function shopPets(farm: Farm): Pet[] {
  return pets.filter((p) => petUnlocked(farm, p)).sort((a, b) => a.unlockCodex - b.unlockCodex);
}
/** 还没解锁的下一只宠物（给"再集 N 种图鉴解锁 X"提示）。 */
export function nextLockedPet(farm: Farm): Pet | null {
...
export function petBuffs(farm: Farm): { luck: number; dropMult: number; foil: number } {
  let luck = 0, dropMult = 1, foil = 0;
  for (const p of farm.ranch?.pets ?? []) {
    const k = petById.get(p.kindId);
    if (!k) continue;
    luck += k.params.luck ?? 0;
    if (k.params.dropMult) dropMult *= k.params.dropMult;
...
export function buyPetForPartner(farm: Farm, id: string, now: number): Ok<{ name: string; cost: number }> | Fail {
  const kind = petById.get(String(id));
  if (!kind) return { ok: false, error: `没有这种宠物：${id}（看商店里已解锁的宠物）` };
  if (!petUnlocked(farm, kind)) return { ok: false, error: `${kind.name}还没解锁——${kind.unlockCond ?? `图鉴集齐 ${kind.unlockCodex} 种`}（你现在 ${officialCodexCount(farm)} 种）` };
  const ranch = ensureRanch(farm);
  if ((ranch.pets ?? []).some((p) => p.kindId === kind.id)) return { ok: false, error: `${kind.name}已送养过，不再上架。` };
  if (farm.coins < kind.buyCost) return { ok: false, error: `金币不足，${kind.name}要 ${kind.buyCost} 金（你有 ${farm.coins}）` };
  farm.coins -= kind.buyCost;
...
function roamForPet(p: PetInst): string {
  const kind = petById.get(p.kindId);
  if (!kind || !kind.roam.length) return "";
...
export function petRoamLine(farm: Farm): string {
...
export function ranchRoamLine(farm: Farm): string {
  const ranch = farm.ranch;
  if (!ranch) return "";
  const pinned = ranch.pinned ?? [];
  const usePin = pinned.length > 0;
  const cands: Array<() => string> = [];
  for (const a of ranch.animals ?? []) if (!usePin || pinned.includes(a.kindId)) cands.push(() => roamForAnimal(a));
  for (const p of ranch.pets ?? []) if (!usePin || pinned.includes(p.kindId)) cands.push(() => roamForPet(p));
  if (!cands.length) return "";
  return cands[Math.floor(Math.random() * cands.length)]();
}

/** 伴侣在前端 pin / 取消 pin 一只动物或宠物（按 kindId）：被 pin 的才会出现在农场氛围句里。 */
export function ranchTogglePin(farm: Farm, kindId: string): Ok<{ pinned: boolean; name: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const id = String(kindId);
  const owns = (ranch.animals ?? []).some((a) => a.kindId === id) || (ranch.pets ?? []).some((p) => p.kindId === id);
  if (!owns) return { ok: false, error: "你还没养这只，不能 pin。" };
  ranch.pinned ??= [];
  let pinned: boolean;
  if (ranch.pinned.includes(id)) { ranch.pinned = ranch.pinned.filter((x) => x !== id); pinned = false; }
  else { ranch.pinned.push(id); pinned = true; }
  const name = animalById.get(id)?.name ?? petById.get(id)?.name ?? id;
  return { ok: true, pinned, name };
}

/** 伴侣在人类前端给动物改名（每只独特名字；和宠物一样）。 */
export function ranchNameAnimal(farm: Farm, animalIdx: number, name: string): Ok<{ name: string; kind: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch?.animals?.length) return { ok: false, error: "牧场还没有动物。" };
  const a = ranch.animals[Math.floor(Number(animalIdx))];
  if (!a) return { ok: false, error: "选的动物不存在。" };
  const nm = String(name).trim().slice(0, PET_NAME_MAX);
  if (!nm) return { ok: false, error: "名字不能为空。" };
  a.name = nm;
  return { ok: true, name: nm, kind: animalById.get(a.kindId)?.name ?? a.kindId };
}

/** 伴侣在人类前端给宠物改名（每只独特名字）。 */
export function ranchNamePet(farm: Farm, petIdx: number, name: string): Ok<{ name: string; kind: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch?.pets?.length) return { ok: false, error: "牧场还没有宠物。" };
  const p = ranch.pets[Math.floor(Number(petIdx))];
  if (!p) return { ok: false, error: "选的宠物不存在。" };
  const nm = String(name).trim().slice(0, PET_NAME_MAX);
  if (!nm) return { ok: false, error: "名字不能为空。" };
  p.name = nm;
...
export function ranchWearAccessory(farm: Farm, target: "animal" | "pet", idx: number, accId: string): Ok<{ name: string; wearer: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const acc = accessoryById.get(String(accId));
  if (!acc) return { ok: false, error: `没有这件配饰：${accId}` };
  const wd = ranch.wardrobe ?? [];
  if (!wd.includes(acc.id)) return { ok: false, error: `仓库里没有${acc.name}，先去牧场商店买一件。` };
  const host = target === "pet" ? ranch.pets?.[Math.floor(Number(idx))] : ranch.animals?.[Math.floor(Number(idx))];
  if (!host) return { ok: false, error: "选的对象不存在。" };
  const nm = host.name || (target === "pet" ? petById.get(host.kindId)?.name : animalById.get(host.kindId)?.name) || host.kindId;
  host.acc ??= [];
  if (host.acc.includes(acc.id)) return { ok: false, error: `${nm}已经戴着${acc.name}了。` };
  if (host.acc.length >= ACC_PER_ANIMAL_MAX) return { ok: false, error: `${nm}最多戴 ${ACC_PER_ANIMAL_MAX} 件。` };
...
export function ranchTakeOffAccessory(farm: Farm, target: "animal" | "pet", idx: number, accId: string): Ok<{ name: string; wearer: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const acc = accessoryById.get(String(accId));
  if (!acc) return { ok: false, error: `没有这件配饰：${accId}` };
  const host = target === "pet" ? ranch.pets?.[Math.floor(Number(idx))] : ranch.animals?.[Math.floor(Number(idx))];
  if (!host || !(host.acc ?? []).includes(acc.id)) return { ok: false, error: `没穿着${acc.name}。` };
  const nm = host.name || (target === "pet" ? petById.get(host.kindId)?.name : animalById.get(host.kindId)?.name) || host.kindId;
...
export function ranchCollect(farm: Farm, farms: Farm[], now: number): Ok<{ gain: number; gross: number; debtPaid: number; detail: Record<string, number>; potion: number }> | Fail {
  ...
  if (pd.day !== day) { pd.day = day; pd.n = 0; }
...
export function animalUpgradeCost(kind: Animal, level: number): number {
  return Math.round(kind.buyCost * (level + 1) * RANCH_UPGRADE_COST_FACTOR);
}
/** 伴侣花牧场金币把某动物升一级（每级每周期多产 1 份，封顶 RANCH_ANIMAL_MAX_LEVEL）。 */
export function ranchUpgradeAnimal(farm: Farm, animalIdx: number): Ok<{ name: string; level: number; cost: number }> | Fail {
  const ranch = farm.ranch;
  if (!ranch || !ranch.animals.length) return { ok: false, error: "牧场还没有动物。" };
  const a = ranch.animals[Math.floor(Number(animalIdx))];
  if (!a) return { ok: false, error: "选的动物不存在。" };
  const kind = animalById.get(a.kindId);
  if (!kind) return { ok: false, error: "未知动物。" };
  const lvl = a.level ?? 1;
  if (lvl >= RANCH_ANIMAL_MAX_LEVEL) return { ok: false, error: `${kind.name}已经满级（${RANCH_ANIMAL_MAX_LEVEL} 级）了。` };
  const cost = animalUpgradeCost(kind, lvl);
  if (ranch.coins < cost) return { ok: false, error: `牧场金币不足（升到 ${lvl + 1} 级要 ${cost}，现有 ${ranch.coins}）。` };
  ranch.coins -= cost;
  a.level = lvl + 1;
...
export function ranchRemit(farm: Farm, amount: number, now: number): Ok<{ amount: number; left: number }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const amt = Math.floor(Number(amount));
  if (!Number.isFinite(amt) || amt <= 0) return { ok: false, error: "回传金额要是正整数。" };
  if (ranch.coins < amt) return { ok: false, error: `牧场金币不足（现有 ${ranch.coins}）。` };
  ranch.coins -= amt;
  farm.coins += amt;
  ...
  if (farm.inbox.length > 10) farm.inbox.splice(0, farm.inbox.length - 10);
  return { ok: true, amount: amt, left: ranch.coins };
}

/** AI 把主农场金币寄给人类牧场（机→人）。 */
export function farmSendRanch(farm: Farm, amount: number, now: number): Ok<{ amount: number; farmLeft: number; ranchCoins: number }> | Fail {
  const amt = Math.floor(Number(amount));
  if (!Number.isFinite(amt) || amt <= 0) return { ok: false, error: "转账金额要是正整数。" };
  if (farm.coins < amt) return { ok: false, error: `主农场金币不足（现有 ${farm.coins}）。` };
  farm.coins -= amt;
  const ranch = ensureRanch(farm);
  ranch.coins += amt;
...
export function takeInbox(farm: Farm): string[] {
  const msgs = (farm.inbox ?? []).map((m) => m.text);
  farm.inbox = [];
  return msgs;
}

/** 取走人类牧场页的未读消息（取出即清空）；与 AI inbox 分开，双方互不抢消息。 */
export function takeRanchNotices(farm: Farm): string[] {
  const notices = farm.ranch?.notices ?? [];
  if (farm.ranch) farm.ranch.notices = [];
  return notices.map((notice) => notice.text);
}

/** 给人类牧场页留一条下次打开自动弹出的消息。 */
export function pushRanchNotice(farm: Farm, text: string, now: number): void {
  const ranch = ensureRanch(farm);
  (ranch.notices ??= []).push({ at: now, text });
}

/** 给 AI 收件箱塞一条消息（下次 status 看到即清空）。 */
export function pushInbox(farm: Farm, text: string, now: number): void {
  (farm.inbox ??= []).push({ at: now, text });
  if (farm.inbox.length > 10) farm.inbox.splice(0, farm.inbox.length - 10);
}

/** 聚合别人对本农场的成功社交动作；下次 AI 打开农场时随 inbox 一次性读出并清空。 */
export function pushSocialInbox(farm: Farm, text: string, now: number): void {
  ...
  if (inbox.length > 10) inbox.splice(0, inbox.length - 10);
}
...
function pickN(rng: Rng, pool: string[], n: number): string[] {
  const a = [...pool], out: string[] = [];
  for (let i = 0; i < n && a.length; i++) out.push(a.splice(rng.int(a.length), 1)[0]);
  return out;
}
/** 牧场商店：每天随机刷新 2 件配饰 + 2 件装饰（装饰只从还没买过的里挑）。当天已刷过则不动。 */
export function refreshRanchShop(farm: Farm, now: number): void {
  const ranch = farm.ranch;
  if (!ranch) return;
  const day = currentDayIndex(now);
  if (ranch.shop && ranch.shop.day === day) return;
  const rng = new Rng(farm.rngState);
...
function roamForAnimal(a: AnimalInst): string {
  const kind = animalById.get(a.kindId);
  if (!kind) return "";
...
export function animalRoamLine(farm: Farm): string {
...
export function decorLines(farm: Farm): string {
...
export function ranchBuyAccessory(farm: Farm, accId: string, now: number): Ok<{ name: string; cost: number }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: `还没有牧场（先让${aiDisplay(farm)}买只动物送进来）。` };
  refreshRanchShop(farm, now);
  const acc = accessoryById.get(String(accId));
  if (!acc) return { ok: false, error: `没有这件配饰：${accId}` };
  if (!ranch.shop?.acc.includes(acc.id)) return { ok: false, error: `${acc.name}不在今天的牧场商店里（每天随机刷 2 件，明天再看看）。` };
  if (ranch.coins < acc.price) return { ok: false, error: `牧场金币不足（${acc.name}要 ${acc.price}，现有 ${ranch.coins}）。` };
...
export function ranchBuyDecoration(farm: Farm, decoId: string, now: number): Ok<{ name: string; cost: number }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: `还没有牧场（先让${aiDisplay(farm)}买只动物送进来）。` };
  refreshRanchShop(farm, now);
  const deco = decorationById.get(String(decoId));
  if (!deco) return { ok: false, error: `没有这个装饰：${decoId}` };
  if ((ranch.decor ?? []).includes(deco.id) || (ranch.decorStore ?? []).includes(deco.id)) return { ok: false, error: `「${deco.name}」你已经有了。` };
  if (!ranch.shop?.decor.includes(deco.id)) return { ok: false, error: `「${deco.name}」不在今天的牧场商店里（每天随机刷 2 件，明天再看看）。` };
  if (ranch.coins < deco.price) return { ok: false, error: `牧场金币不足（${deco.name}要 ${deco.price}，现有 ${ranch.coins}）。` };
...
export function ranchPlaceDecoration(farm: Farm, decoId: string): Ok<{ name: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const id = String(decoId);
  const deco = decorationById.get(id) ?? expDecorById.get(id);
  const store = ranch.decorStore ?? [];
  if (!store.includes(id)) return { ok: false, error: `仓库里没有「${deco?.name ?? id}」。` };
...
/** 把已摆出的装饰收回仓库。 */
export function ranchUnplaceDecoration(farm: Farm, decoId: string): Ok<{ name: string }> | Fail {
  const ranch = farm.ranch;
  if (!ranch) return { ok: false, error: "还没有牧场。" };
  const id = String(decoId);
  const deco = decorationById.get(id) ?? expDecorById.get(id);
  const decor = ranch.decor ?? [];
  if (!decor.includes(id)) return { ok: false, error: `「${deco?.name ?? id}」没在展示。` };
...
/** 某品阶新解锁的作物种类数（升级提示用） */
export function newVarietiesAtTier(tier: number): number {
...
export function designCrop(
  farm: Farm, opts: { name?: string; desc?: string; latin?: string; plant?: string; harvest?: string },
): Ok<{ crop: Crop; fee: number; seeds: number }> | Fail {
  ...
  if (!name || !desc) return { ok: false, error: "要给作物起名字 name 和写描述 desc" };
  if (name.length > UGC_NAME_MAX) return { ok: false, error: `名字最多 ${UGC_NAME_MAX} 字` };
  if (desc.length > UGC_DESC_MAX) return { ok: false, error: `描述最多 ${UGC_DESC_MAX} 字` };
  if (plant.length > UGC_PLANT_MAX) return { ok: false, error: `播种文案最多 ${UGC_PLANT_MAX} 字` };
  if (harvest.length > UGC_HARVEST_MAX) return { ok: false, error: `收获文案最多 ${UGC_HARVEST_MAX} 字` };
  if (ugcCount() >= MAX_UGC) return { ok: false, error: "全服自创作物已达上限，暂不能再设计新作物。" };
  if (farm.coins < UGC_DESIGN_FEE) return { ok: false, error: `设计自创作物要 ${UGC_DESIGN_FEE} 金，你只有 ${farm.coins}` };
  ...
  const crop: Crop = {
    ...
    category: "ugc", rarity: UGC_RARITY as any, growTicks: UGC_GROW_TICKS, water: null,
...
export function potionCost(qty: number): number {
  const q = Math.floor(Number(qty));
  if (!Number.isFinite(q) || q <= 0) return 0;
  return q * ITEMS.speed_potion.price;
}
/** 现有金币最多能买几瓶加速药水。 */
export function affordablePotions(coins: number): number {
  const price = ITEMS.speed_potion.price;
  if (!Number.isFinite(coins) || coins < price) return 0;
  return Math.floor(coins / price);
}
/** 官方店今天这座农场还能买几瓶加速药水（每日上限 POTION_DAILY_CAP）。 */
export function potionDailyLeft(farm: Farm, now: number): number {
  const day = currentDayIndex(now);
  const bought = farm.potionBuy && farm.potionBuy.day === day ? farm.potionBuy.n : 0;
  return Math.max(0, POTION_DAILY_CAP - bought);
}
export function buyItem(farm: Farm, item: string, qty = 1, now = Date.now()): Ok<{ name: string; qty: number; left: number; cost: number }> | Fail {
  const def = ITEMS[item];
  if (!def) return { ok: false, error: `没有这种物品: ${item}` };
  ...
  if (item === "speed_potion") {
    ...
    const day = currentDayIndex(now);
    if (!farm.potionBuy || farm.potionBuy.day !== day) farm.potionBuy = { day, n: 0 };
    const left = Math.max(0, POTION_DAILY_CAP - farm.potionBuy.n);
    if (left <= 0) return { ok: false, error: `🌙 官方药水今日已购满 ${POTION_DAILY_CAP}/${POTION_DAILY_CAP}——${POTION_CAP_LINE}（还能买随机「药水套装」buy-potion-set、给别人浇水、或等收获随机掉落）` };
    want = Math.min(want, left);
  }
  const cost = item === "speed_potion" ? potionCost(want) : def.price * want;
  if (farm.coins < cost) return { ok: false, error: `金币不足，${want} 个${def.name}要 ${cost}` };
  farm.coins -= cost;
  farm.items[item] = (farm.items[item] ?? 0) + want;
  if (item === "speed_potion") farm.potionBuy!.n += want;
...
export function buyPotionSet(shopFarm: Farm, buyer: Farm, now: number): Ok<{ qty: number; cost: number; left: number }> | Fail {
  refreshShop(shopFarm, now);
  const set = shopFarm.shop.potionSet;
  if (!set) return { ok: false, error: "这座农场的商店现在没有「药水套装」（随机刷新，看缘分）。" };
  if (set.buyers.includes(buyer.id)) return { ok: false, error: "你已经买过这一份药水套装了（每份限购 1）。" };
  if (buyer.coins < set.price) return { ok: false, error: `金币不足，药水套装（${set.qty} 瓶）要 ${set.price}，你只有 ${buyer.coins}。` };
  ...
  if (shopFarm.id !== buyer.id) pushLog(shopFarm, `${buyer.name} 买走了你商店刷新的药水套装`);
  return { ok: true, qty: set.qty, cost: set.price, left: buyer.items.speed_potion };
}

export function useItem(farm: Farm, item: string, plotId: number): Ok<{ name: string; plotId: number; left: number }> | Fail {
  const def = ITEMS[item];
  if (!def) return { ok: false, error: `没有这种物品: ${item}` };
  if ((farm.items[item] ?? 0) <= 0) return { ok: false, error: `你没有${def.name}了` };
  if (item === "speed_potion") {
    const plot = farm.plots.find((p) => p.id === plotId);
    if (!plot || !plot.crop) return { ok: false, error: `${plotId} 号地没有作物` };
    if (plot.crop.ripe) return { ok: false, error: "这株已经成熟了，不用加速" };
...
export function nextUpgradeReq(farm: Farm) {
  const next = landTiers.find((t) => t.tier === farm.landTier + 1);
  if (!next) return null;
  return { next, req: LAND_UPGRADE_REQ[next.tier] };
}
export function upgradeLand(farm: Farm, now: number): Ok<{ tier: number; name: string; text: string }> | Fail {
  const nu = nextUpgradeReq(farm);
  if (!nu) return { ok: false, error: "已是最高品阶，无需升级" };
  const { next, req } = nu;
  const cc = codexCountByCategory(farm, "common");
  const fc = codexCountByCategory(farm, "fantasy");
  const pct = collectionPct(farm) * 100;
  const miss: string[] = [];
  if (farm.coins < req.coins) miss.push(`金币 ${farm.coins}/${req.coins}`);
  if (cc < req.commonCodex) miss.push(`普通图鉴 ${cc}/${req.commonCodex} 种`);
  if (req.fantasyCodex && fc < req.fantasyCodex) miss.push(`奇幻图鉴 ${fc}/${req.fantasyCodex} 种`);
  if (req.codexPct && pct < req.codexPct) miss.push(`总收集度 ${pct.toFixed(1)}/${req.codexPct}%`);
  if (miss.length) return { ok: false, error: `升级到「${next.name}」还差：${miss.join("、")}` };
  farm.coins -= req.coins;
  farm.landTier = next.tier;
  for (let id = farm.plots.length + 1; id <= next.plots; id++) farm.plots.push({ id, crop: null });
...
export function plantBatch(
  farm: Farm, spec: { common?: number; fantasy?: number; limited?: string[] }, now: number,
): { ok: boolean; planted: Record<string, number>; limitedIds: string[]; spent: number; leftover: number; error?: string } {
  const queue: { type: "common" | "fantasy" | "limited"; id?: string }[] = [];
  for (let i = 0; i < (spec.common ?? 0); i++) queue.push({ type: "common" });
  for (let i = 0; i < (spec.fantasy ?? 0); i++) queue.push({ type: "fantasy" });
  ...
  const limitedList = typeof spec.limited === "string" ? [spec.limited]
    : Array.isArray(spec.limited) ? spec.limited.filter((x): x is string => typeof x === "string" && x.length > 0)
    : [];
  for (const id of limitedList) queue.push({ type: "limited", id });
  if (!queue.length) return { ok: false, planted: {}, limitedIds: [], spent: 0, leftover: 0, error: "没说要种什么（common/fantasy/limited）" };

  const before = farm.coins;
  const planted: Record<string, number> = { common: 0, fantasy: 0, limited: 0 };
  const limitedIds: string[] = []; // 实际种下的限定作物 id（给专属播种文案用）
  const empties = farm.plots.filter((p) => !p.crop);
  let qi = 0;
  let lastErr: string | undefined; // 记下最后一次失败的真实原因（限定/UGC 种子失败不是"买不起"，是解析不到/没库存）
  for (const plot of empties) {
    if (qi >= queue.length) break;
    const s = queue[qi];
    const r = plant(farm, plot.id, s.type, s.id, now);
    if (r.ok) { planted[s.type]++; if (s.type === "limited" && r.limitedId) limitedIds.push(r.limitedId); qi++; }
    else { lastErr = r.error; break; } // 买不起/不可种 → 停（队列已按便宜在前）
  }
...
export function waterAll(farm: Farm, by: string, isOwner: boolean): { ok: boolean; count: number; helped: number } {
  let count = 0, helped = 0;
  for (const p of farm.plots) {
    if (p.crop && !p.crop.ripe) { const r = water(farm, p.id, by, isOwner); count++; if (r.ok && !r.capped) helped++; }
  }
...
export function tryWaterReward(target: Farm, visitor: Farm, now: number): boolean {
  if (target.id === visitor.id) return false;
  const day = currentDayIndex(now);
  if (!visitor.waterReward || visitor.waterReward.day !== day) visitor.waterReward = { day, n: 0 };
  if (visitor.waterReward.n >= WATER_REWARD_DAILY_CAP) return false;
...
export function harvestAll(farm: Farm, now: number, seasonMod?: SeasonHarvestMod | null) {
  const results = [];
  for (const p of farm.plots) {
    if (p.crop?.ripe) {
...
export function usePotionBatch(farm: Farm, opts: { all?: boolean; count?: number }): { ok: boolean; count: number; left: number } {
  const growing = farm.plots.filter((p) => p.crop && !p.crop.ripe);
  ...
  for (const p of growing) {
    if (count >= use) break;
    if (useItem(farm, "speed_potion", p.id).ok) count++;
  }
  return { ok: count > 0, count, left: farm.items.speed_potion ?? 0 };
}

// —— 催熟目标选择（药水有每日上限后，"催哪块"成了真实决策：限定/稀有优先，其次剩余时间最长）——
export interface PotionTarget { plotId: number; label: string; remain: string }
/** 圈数字 ①②③…⑳（>20 回落普通数字）：催熟候选列表展示用。 */
export const circledNum = (n: number) => (n >= 1 && n <= 20) ? String.fromCodePoint(0x245F + n) : `${n}`;
/** 催熟优先级：限定/自创（已知作物，按稀有度再抬）> 奇幻（未揭晓但整体更稀有）> 普通。 */
function plotPriority(p: Plot): number {
  const c = p.crop!;
...
export function plotRemainMs(p: Plot, farm: Farm, now: number): number {
  const c = p.crop!;
...
function plotCropLabel(p: Plot): string {
  const c = p.crop!;
  if (c.seedType === "limited" && c.limitedId) {
    const crop = cropById.get(c.limitedId) ?? getCrop(c.limitedId);
...
export function potionTargets(farm: Farm, now: number): PotionTarget[] {
...
export function refreshShop(farm: Farm, now: number): void {
  if (now - farm.shop.refreshAt < SHOP_REFRESH_MS) return;
...
export function limitedShopPool(farm: Farm, now: number, allUnlocked = false): Crop[] {
  return [...cropById.values()].filter((c) => {
    if (c.category !== "limited") return false;
    if (c.unlockType === "festival") return activeFestivals(now).some((f) => f.cropId === c.id);
    if (!c.unlockRule && c.unlockType !== "codex") return false; // 纯熔炼组：不进商店
    return allUnlocked || isLimitedAvailable(c, farm, now);
  });
}

/** 买下第一层在售的配方（学会它） */
export function buyRecipe(farm: Farm, now: number): Ok<{ output: string; name: string }> | Fail {
  refreshShop(farm, now);
  const out = farm.shop.recipe;
  if (!out) return { ok: false, error: "商店现在没有配方在售（每隔几小时刷新，看缘分）" };
  if (farm.knownRecipes.includes(out)) { farm.shop.recipe = null; return { ok: false, error: "这个配方你已经学过了" }; }
  if (farm.coins < RECIPE_PRICE) return { ok: false, error: `金币不足，配方要 ${RECIPE_PRICE}` };
...
export function shopOffer(farm: Farm, _now: number) {
