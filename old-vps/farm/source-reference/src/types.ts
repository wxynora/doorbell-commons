...
/** 人类牧场页的当日派遣记录；只保留当前日桶，结果与实际金币结算一致。 */
export interface RanchRaidHistoryEntry {
  raidId: string;
  animalName: string;
  targetFarmId: string;
  targetName: string;
  startedAt: number;
  status: "active" | "caught" | "returned";
  coins?: number;
}
...
export interface Ranch {
  ...
  /** 动物偷金币的当日实际到账累计（day=UTC+8 日序号；每天最多 500 金）。 */
  raidIncome?: { day: number; n: number };
  /** 动物偷金币玩法的当日实际损失累计（被偷及被抓赔款；含形成隐形欠款的部分；day=UTC+8 日序号）。 */
  raidLoss?: { day: number; n: number };
  /** UTC+8 当日派遣记录；次日首次派遣时换成新日桶，不截断当日记录。 */
  raidHistory?: { day: number; entries: RanchRaidHistoryEntry[] };
  /** 人类前端下次打开时一次性弹出的牧场消息；与 AI 的 farm.inbox 独立消费。 */
  notices?: { at: number; text: string }[];
...
export interface LedgerEntry {
  at: number;
  /** buy-animal=AI 买动物送人 / buy-pet=AI 买宠物送人 / remit=人回传金币 / send-ranch=AI 给牧场寄金币 / potion=人收获掉药水入库 */
  type: "buy-animal" | "buy-pet" | "remit" | "send-ranch" | "potion";
  /** 金币数（buy-animal/buy-pet/remit/send-ranch）或瓶数（potion）*/
  amount: number;
  note: string;
}
