...
export const GROW_TICKS: Record<string, number> = { common: 6, fantasy: 12 }; // 3h / 6h
// 限定作物用作物自带 growTicks。
...
// —— 道具（最小消耗品系统，将来可扩展）——
export const ITEMS: Record<string, { name: string; price: number; desc: string }> = {
...
// —— 种子价 ——
export const SEED_PRICE: Record<string, number> = { common: 8, fantasy: 40 };

// —— 抽卡 ——
// 各稀有度基础权重（越稀有越低）
export const RARITY_WEIGHT: Record<string, number> = { N: 100, R: 35, SR: 10, SSR: 2.5, SP: 0.4 };
export const RARITY_ORDER = ["N", "R", "SR", "SSR", "SP"] as const;
export const rarityIndex = (r: string) => Math.max(0, RARITY_ORDER.indexOf(r as any));
// 土地品阶的“运气”加成（roll 时把权重往高稀有抬）。顶阶=4阶（已砍5阶），4阶给满 1.5 补回稀有概率手感。
export const LAND_LUCK: Record<number, number> = { 1: 0.15, 2: 0.45, 3: 0.9, 4: 1.5 };
...
export const LAND_UPGRADE_REQ: Record<number, { coins: number; commonCodex: number; fantasyCodex: number; codexPct: number }> = {
...
export const NEW_CODEX_REWARD: Record<string, number> = { N: 30, R: 80, SR: 250, SSR: 800, SP: 2500 };
...
export const MATERIAL_DROP_WEIGHT: Record<string, number> = { N: 100, R: 55, SR: 28, SSR: 12, SP: 3 };

// —— 熔炼（投素材 → 出限定种子）——
export const CRAFT_COUNT = 3; // 一次熔炼投入的素材数
// 素材稀有度 → 熔炼点数（点数越高，越容易熔出稀有限定）
export const FUSION_POINTS: Record<string, number> = { N: 1, R: 3, SR: 8, SSR: 20, SP: 50 };
// 限定作物各稀有度的基础权重（熔炼随机产出用；点数高会往高稀有抬）
export const LIMITED_BASE_WEIGHT: Record<string, number> = { SR: 100, SSR: 25, SP: 3 };
...
export const MATERIAL_REF_PRICE: Record<string, number> = { N: 10, R: 30, SR: 80, SSR: 250, SP: 800 };
...
export const EXP_DC: Record<string, number> = { easy: 6, mid: 8, hard: 9 }; // 难度档→2d6 目标
