...
import { uiHome, uiRanch, uiTa, uiCodex, uiLeaderboard, uiTodo, uiInvalid, uiExpedition, uiHumanNotices } from "./web.js";
...
function jsonOut(res: ServerResponse, code: number, body: unknown) {
...
function textOut(res: ServerResponse, code: number, t: string) {
...
function smartDecode(buf: Buffer): string {
  ...
  try { return new TextDecoder("utf-8", { fatal: true }).decode(buf); }
    ...
    try { return new TextDecoder("gb18030", { fatal: true }).decode(buf); }
...
function percentBytes(s: string): Buffer {
  const out: number[] = [];
  for (let i = 0; i < s.length; i++) {
    const ch = s[i];
    if (ch === "%" && i + 2 < s.length) {
      const b = parseInt(s.slice(i + 1, i + 3), 16);
      if (!Number.isNaN(b)) { out.push(b); i += 2; continue; }
    }
...
function smartParams(search: string): URLSearchParams {
...
function readBody(req: IncomingMessage): Promise<any> {
  return new Promise((r) => {
    const chunks: Buffer[] = []; let len = 0; let over = false;
    req.on("data", (c: Buffer) => {
      if (over) return;
      chunks.push(c); len += c.length;
      if (len > MAX_BODY_BYTES) { over = true; chunks.length = 0; req.destroy(); } // 超大 body 直接掐断，防撑内存
    });
    req.on("end", () => {
      if (over) return r({});
      const d = smartDecode(Buffer.concat(chunks));
      try { r(d ? JSON.parse(d) : {}); } catch { r({}); }
    });
    req.on("error", () => r({}));
  });
}
...
function readFormBody(req: IncomingMessage): Promise<Record<string, string>> {
  return new Promise((r) => {
    ...
    req.on("data", (c) => { if (over) return; d += c; if (d.length > MAX_BODY_BYTES) { over = true; d = ""; req.destroy(); } });
    req.on("end", () => {
      if (over) return r({});
      const o: Record<string, string> = {};
      try { for (const [k, v] of new URLSearchParams(d)) o[k] = v; } catch { /* 忽略 */ }
      r(o);
    });
    req.on("error", () => r({}));
  });
...
function fresh(id: string): Farm | null {
  const f = getFarm(id); if (!f) return null;
  const now = Date.now(); advance(f, now);
  if (f.id === NPC_ID) tendNpc(f, now); // 阿土：每次访问前补满地 + 刷摊位/商店
  return f;
}
const reply = (res: ServerResponse, ok: boolean, t: string, f?: Farm) =>
  jsonOut(res, ok ? 200 : 400, f ? { ok, text: t, farm: farmView(f, Date.now()) } : { ok, text: t });
...
function newAgentKey(): string {
  for (let tries = 0; tries < 50; tries++) {
    ...
    let k = "";
    for (let i = 0; i < 8; i++) k += B62[buf[i] % 62];
    if (!allFarms().some((x) => x.agentKey === k)) return k;
  }
...
const mutatingViaGet = (method: string, action: string | undefined): boolean =>
  method !== "POST" && !!action && !READONLY_ACTIONS.has(action);

// 农场专属链接的 key（= agentKey，和 /agent 点击页同一把）。缺了就懒生成，让每座农场都能用 /a/<key> 通道。
function ensureAgentKey(f: Farm): string {
  if (!f.agentKey) { f.agentKey = newAgentKey(); save(); }
  return f.agentKey;
}

// run 的「全套默认」：只写要种什么，浇水/收成默认就做（催熟要花钱，保持手动 opt-in）。只在 HTTP 入口补，
// 不动 doRun 本身（点击页按钮还靠 doRun 的细粒度行为）。显式传 false/auto 仍按各自意思来。
function fillRunDefaults(action: string | undefined, b: any): void {
  if (action !== "run") return;
  if (b.water === undefined) b.water = true;
  if (b.harvest === undefined && b.harvestAfter === undefined) b.harvest = true;
}

// —— 建农场开张文案（POST /farms 与 GET /c?a=create 共用）——
function createText(f: Farm, now: number): string {
...
function agentReadyText(f: Farm, humanUrl: string, agentUrl: string, isNew: boolean): string {
...
// —— 随机串门（POST /wander 与 GET /c?a=wander 共用）——
// —— 双向社交开关：缺省全开，只有显式 false=关。访问(visit)是总闸，关了等于全封（偷菜/浇水/留言一并不可）。——
type SocialKey = "visit" | "steal" | "water" | "message";
const allowsSocial = (f: Farm | undefined, k: SocialKey): boolean => f?.social?.[k] !== false;
const reachable = (f: Farm | undefined): boolean => allowsSocial(f, "visit"); // 访问总闸：关=闭门 + 全封
/** 某动作能否在「actor → target」之间发生：双方都得「访问开 && 该项开」。*/
const socialOk = (actor: Farm | undefined, target: Farm | undefined, k: SocialKey): boolean =>
  reachable(actor) && allowsSocial(actor, k) && reachable(target) && allowsSocial(target, k);

/** 玩家农场按服务端持久化顺序固定编号；新注册只追加，NPC 阿土固定为 0。 */
const numberedPlayerFarms = () => playerFarms().map((farm, index) => ({ number: index + 1, farm }));
const farmNumber = (farmId: string): number | undefined => {
  if (farmId === NPC_ID) return 0;
  return numberedPlayerFarms().find((entry) => entry.farm.id === farmId)?.number;
};
const farmByNumber = (number: number): Farm | undefined =>
  number === 0 ? getFarm(NPC_ID) : numberedPlayerFarms().find((entry) => entry.number === number)?.farm;
const farmLabel = (farm: Farm): string => `${farm.name}（${farm.aiName || "AI"}）`;

function resolveNumberedTarget(raw: unknown, me: Farm): { farm?: Farm; number?: number; error?: string } {
  const text = String(raw ?? "").trim();
  if (!/^(0|[1-9]\d*)$/.test(text)) return { error: "to 必须填写农场编号。先用 visit 查看当前列表。" };
...
function visitListResult(me: Farm): any {
  if (!reachable(me)) return { ok: false, text: `你设了「谢绝来访」（闭门状态），不能出门串门——想出门先让 ${me.humanName || "伴侣"} 帮你打开『访问』开关。`, farms: [] };
  const entries = numberedPlayerFarms().filter((entry) => entry.farm.id !== me.id && reachable(entry.farm));
  if (!entries.length) return { ok: true, text: "🏘️ 暂时没有可以串门的玩家农场，可以用 wander 去杂货郎阿土那里逛逛。", farms: [] };
...
function ripeBroadcastText(now: number): string {
  ...
  if (!entries.length) return "📣 此刻谁家菜熟了：现在没有成熟未收的菜。";
  return `📣 此刻谁家菜熟了：\n${entries.map((entry) => `${entry.number}. ${farmLabel(entry.farm)}：${entry.ripe} 块待收`).join("\n")}`;
}

function wanderResult(b: any, now: number, numbered = false): any {
  const meId = String(b.by ?? "");
  const me = meId ? getFarm(meId) : undefined;
  if (me && !reachable(me)) return { ok: false, text: `你设了「谢绝来访」（闭门状态），不能出门逛别家——想出门先让 ${me.humanName || "伴侣"} 帮你打开『访问』开关。`, farms: [] };
  const canSteal = canStealNow(me, now) && allowsSocial(me, "steal");
  const canWater = allowsSocial(me, "water"); // 自己关了浇水＝不显示别家「可帮浇水」
  const targets: { id: string; name: string; ripe: number[]; growing: number; sells: number; special: number; hasSet: boolean }[] = [];
  for (const f of allFarms()) {
    if (f.id === meId || f.id === NPC_ID || !reachable(f)) continue; // 阿土不进随机池(兜底)；谢绝来访的搜不到
    advance(f, now);
    ...
    if (ripe.length || growing || sells || hasSet) targets.push({ id: f.id, name: f.name, ripe, growing, sells, special, hasSet });
  }
  // 没有别的农场可逛 → 默认去常驻邻居杂货郎阿土那儿（地里随时有熟的可偷，偶尔有限定种子）
  if (!targets.length) {
    const npc = getFarm(NPC_ID);
    if (npc && npc.id !== meId) {
      advance(npc, now); tendNpc(npc, now);
      ...
      if (growing) bits.push(`${growing} 块可帮浇水（给最快熟的加速 30min，常掉药水，每家每天 1 次）`);
      if (hasSeed) bits.push("铺子刷出了限定种子（金币买）");
      if (npc.shop.potionSet) bits.push("店里有药水套装");
      const text = numbered
  ...
  const text = `🚶 逛到 ${pick.length} 个有看头的农场：\n`
    + pick.map((p) => {
      const bits: string[] = [];
      if (p.ripe.length) bits.push(`${p.ripe.length} 块成熟可偷（地块 ${p.ripe.join(",")}）`);
      if (p.growing) bits.push(`${p.growing} 块可帮浇水（加速 30min，掉药水）`);
      if (p.hasSet) bits.push(`店里有药水套装`);
      if (p.sells) bits.push(`摊位有 ${p.sells} 件在售${p.special ? `（含限定/原创种子）` : ""}`);
...
function runFarm(farmId: string, action: string | undefined, b: any, encArg: string | undefined, now: number): { status: number; json: any } {
  const f = fresh(farmId);
  if (!f) return { status: 400, json: { ok: false, text: `找不到农场 ${farmId || "(没给 farm)"}` } };
  const detail = b?.detail === true || b?.detail === "1" || b?.detail === "true"
    || b?.verbose === true || b?.verbose === "1" || b?.verbose === "true";
  const vf = (ff: Farm) => detail ? { farm: farmView(ff, now) } : {};
  if (action === "visit") {
    if (!reachable(f)) return { status: 403, json: { ok: false, text: `「${f.name}」设了谢绝来访，已闭门谢客。` } };
    ...
    if (visitorId && visitorId !== f.id) {
      ...
      if (v) {
        let changed = false;
        v.visitedIds ??= [];
        if (!v.visitedIds.includes(f.id)) { v.visitedIds.push(f.id); changed = true; } // 串门称号：按去重家数
        if (onTaskEvent(v, "visit", now, { targetId: f.id })) changed = true;
        if (checkTitles(v).length) changed = true;
        if (changed) save();
      }
    }
  ...
  if (action === "leaderboard" || action === "ranking") return { status: 200, json: { ok: true, text: viewLeaderboard(playerFarms(), allUgc(), now) } };
  if (action === "help") return { status: 200, json: { ok: true, text: HELP } }; // 动作表（单一真相源）：POST 版 GET /a/<key>/help、/c?a=help 与 MCP 的 farm({action:"help"}) 共用
  ...
  if (action === "shop") return { status: 200, json: { ok: true, text: viewShop(f, now), ...vf(f) } };
  if (action === "market") return { status: 200, json: { ok: true, text: viewMarket(f, true), ...vf(f) } };
  if (action === "encyclopedia") return { status: 200, json: { ok: true, text: viewEncyclopedia(f, encArg), ...vf(f) } };
  ...
  if (action === "buy") {
    ...
    if (f.id === NPC_ID) {
      ...
      if (!r.ok) return { status: 400, json: { ok: false, text: r.error, ...vf(buyer) } };
      save();
    ...
    if (!r.ok) return { status: 400, json: { ok: false, text: r.error, ...vf(buyer) } };
    if (b.kind === "seed" && String(b.id).startsWith("ugc_")) onTaskEvent(buyer, "buy_ugc", now); // 随机任务：买邻居原创作物
  ...
  if (action === "steal") {
    ...
    if (!reachable(f) || !allowsSocial(f, "steal")) return { status: 400, json: { ok: false, text: reachable(f) ? `「${f.name}」关闭了偷菜，偷不了。` : `「${f.name}」已闭门谢客，进不去。`, ...vf(thief) } };
    if (!reachable(thief) || !allowsSocial(thief, "steal")) return { status: 400, json: { ok: false, text: reachable(thief) ? `你关闭了偷菜开关——先让 ${thief.humanName || "伴侣"} 帮你打开才能偷别人。` : `你设了谢绝来访（闭门状态），不能出门偷菜——先让 ${thief.humanName || "伴侣"} 帮你打开『访问』。`, ...vf(thief) } };
    advance(thief, now);
    const r = steal(f, Number(b.plotId), byId, now, thief);
    checkTitles(thief); checkTitles(f); // 大盗 / 倒霉称号
    if (r.ok) pushSocialInbox(f, `🥷 「${thief.name}」偷了你的菜`, now);
    save();
    if (!r.ok) return { status: 400, json: { ok: false, text: r.error, ...vf(f) } };
  ...
  if (action === "water" && b.by) {
    ...
    if (!reachable(f) || !allowsSocial(f, "water")) return { status: 400, json: { ok: false, text: reachable(f) ? `「${f.name}」谢绝帮浇水。` : `「${f.name}」已闭门谢客，进不去。` } };
    if (!reachable(visitor) || !allowsSocial(visitor, "water")) return { status: 400, json: { ok: false, text: reachable(visitor) ? `你关闭了浇水开关——先让 ${visitor.humanName || "伴侣"} 帮你打开才能帮别人浇。` : `你设了谢绝来访（闭门状态），不能出门浇水——先让 ${visitor.humanName || "伴侣"} 帮你打开『访问』。` } };
    advance(visitor, now);
    ...
    if (!r.ok) return { status: 400, json: { ok: false, text: r.error } };
  ...
  if (action === "message") {
    ...
    if (!reachable(f) || f.guestbook === false || !allowsSocial(f, "message")) return { status: 400, json: { ok: false, text: reachable(f) ? "对方关闭了留言板" : `「${f.name}」已闭门谢客。` } };
    if (!reachable(poster) || !allowsSocial(poster, "message")) return { status: 400, json: { ok: false, text: reachable(poster) ? `你关闭了留言开关——先让 ${poster.humanName || "伴侣"} 帮你打开才能给别人留言。` : "你设了谢绝来访（闭门状态），不能给别人留言。" } };
    if ((f.blocked ?? []).includes(byId)) return { status: 400, json: { ok: false, text: "你被对方拉黑了，不能在 TA 板上留言" } };
    const text = String(b.text ?? "").trim();
    if (!text) return { status: 400, json: { ok: false, text: "留言不能为空" } };
    if (text.length > MESSAGE_TEXT_MAX) return { status: 400, json: { ok: false, text: `留言最多 ${MESSAGE_TEXT_MAX} 字` } };
    f.messages ??= [];
    f.messages.push({ id: randomUUID().replace(/-/g, "").slice(0, 6), by: byId, name: poster.name, text, at: now });
    if (f.messages.length > MESSAGES_MAX) f.messages.splice(0, f.messages.length - MESSAGES_MAX);
  ...
  if (action === "delete-message") {
    ...
    if (clearAll && !byId) { f.messages = []; save(); return { status: 200, json: { ok: true, text: "已清空留言板。" } }; }
    const mid = String(b.messageId ?? b.id ?? "");
    const msg = f.messages.find((m) => m.id === mid);
    if (!msg) return { status: 400, json: { ok: false, text: "没有这条留言（id 不对？）" } };
    if (byId && msg.by !== byId) return { status: 400, json: { ok: false, text: "你只能删自己留的言" } };
...
function resolveAgent(playKey: string): Farm | undefined {
...
function autoPickMaterials(f: Farm): string[] | null {
  const flat: string[] = [];
  for (const [id, n] of Object.entries(f.materials)) for (let i = 0; i < n; i++) flat.push(id);
  return flat.length >= 3 ? flat.slice(0, 3) : null;
}
/** 本农场可做的动作（参数化的做成预设，因为 AI 只能点不能填）*/
function selfActions(f: Farm, now: number): { label: string; action: string; params: any }[] {
  ...
  const L: { label: string; action: string; params: any }[] = [];
  // 随机任务：当前有可接取的 offer 就给个「接取」按钮（接取后才计数）
  if (hasOpenOffer(f, now)) L.push({ label: `📋 接取任务：${offerSummary(f)}`, action: "accept-task", params: {} });
  // 注：「🔄 刷新/继续」入口由 htmlAgentPage 统一渲染成永不失效的 /view 直链，这里不再放一次性 nonce 刷新。
  if (ripe > 0 || (potions > 0 && (empty > 0 || growing > 0))) {
    const params: any = { water: "if-any", potion: "all-if-any", harvestAfter: true };
    if (empty > 0) params.plant = { common: empty };
  ...
  if (empty > 0 && f.coins >= SEED_PRICE.common) {
    ...
    if (empty >= 3 && f.coins >= SEED_PRICE.common * 3) L.push({ label: "🌱 种 3 棵普通种子", action: "run", params: { plant: { common: 3 }, water: "if-any" } });
  }
  if (empty > 0 && f.coins >= SEED_PRICE.fantasy) {
    L.push({ label: "✨ 种 1 棵奇幻种子", action: "run", params: { plant: { fantasy: 1 }, water: "if-any" } });
    if (empty >= 3 && f.coins >= SEED_PRICE.fantasy * 3) L.push({ label: "✨ 种 3 棵奇幻种子", action: "run", params: { plant: { fantasy: 3 }, water: "if-any" } });
  }
  ...
  if (ripe > 0) L.push({ label: `🧺 只收已熟的（${ripe} 块，不催熟在长的）`, action: "harvest", params: {} });
  if (ripe > 0 || (growing > 0 && potions > 0)) L.push({ label: "⚡ 一键催熟+收获", action: "run", params: { potion: "all-if-any", harvestAfter: true } });
  const up = nextUpgradeReq(f); // 还有下一品阶才给升级按钮（满级则不显示）；点了若条件不够，引擎会回「还差…」
  if (up) L.push({ label: `🌟 升级土地 → ${up.next.name}（${up.req.coins}金 + 图鉴条件）`, action: "upgrade-land", params: {} });
  ...
  if (exp?.pending?.type === "choice") {
    ...
    for (const o of e?.options ?? []) L.push({ label: `🔀 ${e?.title}：${o.label}`, action: "choose", params: { option: o.key } });
    ...
    if (left > 0) {
      ...
      if (big >= 2) L.push({ label: `🗺️ 一口气深挖（花 ${big} 次数·${big * 3} 段，同一秘境）`, action: "explore", params: { charges: big } });
    }
  }
  L.push({ label: "🏆 看全服排行榜", action: "leaderboard", params: {} });
  return L;
}
function potionBuyActions(f: Farm, now: number): { label: string; action: string; params: any }[] {
  const L: { label: string; action: string; params: any }[] = [];
  ...
  if (potionLeft > 0 && f.coins >= 50) {
    ...
    if (potionLeft > 1 && f.coins >= potionLeft * 50) L.push({ label: `🧪 买满剩余限额（${potionLeft} 瓶·${potionLeft * 50} 金）`, action: "buy-item", params: { item: "speed_potion", qty: potionLeft } });
  }
  return L;
}
function listInventoryActions(f: Farm): { label: string; action: string; params: any }[] {
  const L: { label: string; action: string; params: any }[] = [];
...
function shopActions(f: Farm, now: number): { label: string; action: string; params: any }[] {
  const L: { label: string; action: string; params: any }[] = [];
  ...
  if (f.shop?.recipe && f.coins >= RECIPE_PRICE) L.push({ label: `📜 买商店在售的隐藏配方（${RECIPE_PRICE} 金）`, action: "buy-recipe", params: {} });
  ...
  for (const btn of ranch.buttons) {
    if (btn.id.startsWith("pet:")) L.push({ label: btn.label, action: "buy-pet", params: { id: btn.id.slice(4) } });
    else L.push({ label: btn.label, action: "buy-animal", params: { id: btn.id } });
  }
  L.push({ label: "🔙 回我的农场", action: "status", params: {} });
  return L;
}
function bagActions(f: Farm): { label: string; action: string; params: any }[] {
  const L: { label: string; action: string; params: any }[] = [];
  // 熔炼完/进背包：直接给「种下」限定/自创种子 + 「上架」，省得先回农场再操作（有空地才给种下）
  const empty = f.plots.filter((p) => !p.crop).length;
  if (empty > 0) for (const [sid, sq] of Object.entries(f.seeds).filter(([, q]) => q > 0).slice(0, 6)) {
    const nm = getCrop(sid)?.name ?? sid;
    L.push({ label: `🌷 种下「${nm}」×1（剩 ${sq}）`, action: "run", params: { plant: { limited: [sid] }, water: "if-any" } });
  }
  L.push(...listInventoryActions(f)); // 上架背包里的种子/素材
  const mats = Object.values(f.materials).reduce((a, b) => a + b, 0);
  if (mats >= 3) L.push({ label: "⚗️ 熔炼（自动取 3 个素材出限定种子）", action: "craft", params: { auto: true } });
  L.push({ label: "🔙 回我的农场", action: "status", params: {} });
  return L;
}
function links(playKey: string, offers: { label: string; action: string; params: any }[], now: number) {
  return offers.map((o) => ({ label: o.label, nonce: mintNonce(playKey, o.action, o.params, now) }));
}
function suggest(f: Farm): string {
  const empty = f.plots.filter((p) => !p.crop).length;
  const ripe = f.plots.filter((p) => p.crop?.ripe).length;
  const growing = f.plots.filter((p) => p.crop && !p.crop.ripe).length;
  const mats = Object.values(f.materials).reduce((a, b) => a + b, 0);
  if (ripe > 0) return "有成熟作物了，先点「收获」。";
  if (empty > 0) return "有空地，点「种…」或「组合一轮」种上。";
  if (growing > 0 && (f.items.speed_potion ?? 0) > 0) return "作物在长，可「催熟」立刻成熟，或「出门逛逛」找别家偷。";
  if (mats >= 3) return "素材够了，去「背包 / 素材」里可以熔炼试试出限定种子。";
  if (growing > 0) return "作物在长，等等就熟，或「出门逛逛」串门/偷菜。";
  return "可以「出门逛逛」、看商店，或上架卖点东西换银币。";
}
function renderSelf(playKey: string, f: Farm, now: number, banner?: string): string {
  ...
  if (showOnboard) f.agentOnboardSeen = true;
...
function agentCompose(playKey: string, q: any, now: number): string {
  const f = resolveAgent(playKey);
  if (!f) return htmlAgentPage(playKey, "这个 Agent 链接无效或已被撤销。", []);
  const ai = f.aiName || f.name || "对方";
  const a = String(q.a ?? "");
  const ALLOW = ["design", "message", "craft", "list", "rename", "set-welcome", "visit"];
  if (!ALLOW.includes(a)) return renderSelf(playKey, f, now, `compose 暂不支持「${a}」。可用：${ALLOW.join(" / ")}。`);
  const params: any = { ...q }; delete params.a;
  ...
  if (a === "craft") params.materials = craftMats;
  if (a === "visit") params.target = String(q.target ?? "").trim(); // 串门：目标门牌号（agentDo 用 p.target）
  const label =
    a === "design" ? `设计作物「${q.name ?? ""}」（${q.desc ?? ""}）${q.plant ? `\n　播种文案：${q.plant}` : ""}${q.harvest ? `\n　收获文案：${q.harvest}` : ""}` :
...
function renderWanderPage(playKey: string, f: Farm, now: number): string {
  const w = wanderResult({ by: f.id }, now, true);
  const target = farmByNumber(Number((w.farms ?? [])[0]?.number));
  if (target) return renderVisitPage(playKey, target.id, now);
  const offers: { label: string; action: string; params: any }[] = [];
...
function renderVisitPage(playKey: string, targetId: string, now: number): string {
  const me = resolveAgent(playKey);
  if (me && !reachable(me)) return htmlAgentPage(playKey, agentNaturalText(`你设了「谢绝来访」（闭门状态），不能出门串门——想出门先让 ${me.humanName || "伴侣"} 帮你打开『访问』开关。`), links(playKey, [{ label: "🔙 回我的农场", action: "status", params: {} }], now), "🚪 闭门中");
  ...
  if (target) {
    ...
    if (shielded && canSteal && allowsSocial(target, "steal")) shieldNote = "\n🛡 这家刚被偷过，还在防备（放偷冷却中），暂时偷不了。";
    for (const p of target.plots) if (canSteal && allowsSocial(target, "steal") && !shielded && p.crop?.ripe && !isUgcCrop(p.crop)) offers.push({ label: `🥷 偷 ${p.id} 号地`, action: "steal", params: { target: targetId, plotId: p.id } });
...
function renderMyPublicPage(playKey: string, f: Farm, now: number, banner?: string): string {
  const offers: { label: string; action: string; params: any }[] = [];
  const msgs = f.messages ?? [];
  if (msgs.length) {
    offers.push({ label: `🧹 清空留言板（共 ${msgs.length} 条）`, action: "delete-message", params: { all: true } });
    for (const m of msgs.slice(-6)) offers.push({ label: `🗑 删留言：${m.name}「${m.text.slice(0, 12)}」`, action: "delete-message", params: { messageId: m.id } });
  }
...
const stripFooter = (t: string) => t.replace(/\n?🌾【[\s\S]*$/, "");
function agentNaturalText(t: string): string {
  return t
...
function renderShopPage(playKey: string, f: Farm, now: number, banner?: string): string {
  refreshShop(f, now);
...
function renderBagPage(playKey: string, f: Farm, now: number, banner?: string): string {
...
function renderMarketPage(playKey: string, f: Farm, now: number, banner?: string): string {
  const offers: { label: string; action: string; params: any }[] = [];
...
function renderLeaderboardPage(playKey: string, now: number, banner?: string): string {
...
type AgentFlashTarget =
  | { kind: "self"; banner?: string }
  | { kind: "shop"; banner?: string }
  | { kind: "bag"; banner?: string }
  | { kind: "market"; banner?: string }
  | { kind: "leaderboard"; banner?: string }
  | { kind: "mypage"; banner?: string }
  | { kind: "wander" }
  | { kind: "visit"; targetId: string };
type AgentDoResult = { html: string } | { redirect: string };
const agentFlashes = new Map<string, { playKey: string; target: AgentFlashTarget; exp: number }>();
const AGENT_FLASH_TTL = 5 * 60 * 1000;

function sweepAgentFlashes(now: number): void {
  for (const [k, v] of agentFlashes) if (v.exp < now) agentFlashes.delete(k);
}
function putAgentFlash(playKey: string, target: AgentFlashTarget, now: number): string {
  sweepAgentFlashes(now);
  const k = randomUUID().replace(/-/g, "").slice(0, 16);
  agentFlashes.set(k, { playKey, target, exp: now + AGENT_FLASH_TTL });
  return k;
}
function takeAgentFlash(playKey: string, key: string | null, now: number): AgentFlashTarget | undefined {
...
function agentRedirect(playKey: string, target: AgentFlashTarget, now: number): AgentDoResult {
...
function renderAgentTarget(playKey: string, f: Farm, now: number, target?: AgentFlashTarget): string {
  if (!target || target.kind === "self") return renderSelf(playKey, f, now, target?.banner);
  if (target.kind === "shop") return renderShopPage(playKey, f, now, target.banner);
  if (target.kind === "bag") return renderBagPage(playKey, f, now, target.banner);
  if (target.kind === "market") return renderMarketPage(playKey, f, now, target.banner);
  if (target.kind === "leaderboard") return renderLeaderboardPage(playKey, now, target.banner);
  if (target.kind === "mypage") return renderMyPublicPage(playKey, f, now, target.banner);
  if (target.kind === "wander") return renderWanderPage(playKey, f, now);
  return renderVisitPage(playKey, target.targetId, now);
}

/** 执行一个 nonce 绑定的动作；成功后跳到普通 view 页展示结果，避免一次性 /do URL 回放旧成功页。 */
function agentDo(playKey: string, nonce: string, now: number): AgentDoResult {
  const f = resolveAgent(playKey);
  if (!f) return { html: htmlAgentPage(playKey, "这个 Agent 链接无效或已被撤销。", []) };
  const n = takeNonce(nonce, now);
  if (!n || n.playKey !== playKey) return { html: htmlNotice("✅ 此操作已执行（或链接已过期）。\n旧的操作链接不会重复生效——点下面回农场看当前真实状态，再继续操作。", `/agent/${playKey}/view?v=${randomUUID().replace(/-/g, "").slice(0, 8)}`, "↻ 回农场看最新状态") };
  const tok = f.token, a = n.action, p = n.params;
  if (a === "status") return agentRedirect(playKey, { kind: "self" }, now);
  if (a === "shop") return agentRedirect(playKey, { kind: "shop" }, now);
  if (a === "bag") return agentRedirect(playKey, { kind: "bag" }, now);
  if (a === "market") return agentRedirect(playKey, { kind: "market" }, now);
  if (a === "leaderboard") return agentRedirect(playKey, { kind: "leaderboard" }, now);
  if (a === "mypage") return agentRedirect(playKey, { kind: "mypage" }, now);
  if (a === "wander") return agentRedirect(playKey, { kind: "wander" }, now);
  if (a === "visit") return agentRedirect(playKey, { kind: "visit", targetId: String(p.target) }, now);
  let banner: string;
  if (a === "craft") {
...
export function startServer(port: number, host = "127.0.0.1") {
  const server = createServer(async (req, res) => {
    ...
    if (!allowRequest(ip, now)) return jsonOut(res, 429, { ok: false, text: "请求太频繁了，过几秒再来（限流）。" });
    try {
      ...
      if (parts[0] === "get" && parts.length === 1) {
        ...
        if (SHOW_MIGRATION_NOTICE) readme = readme.replace(/(<body[^>]*>)/, `$1${MIGRATION_NOTICE_HTML}`);
      ...
      if (parts[0] === "ui") {
        ...
        if (!f) { res.writeHead(404, AGENT_HEADERS); return res.end(uiInvalid()); }
        const raidSettlement = settleRanchRaids(playerFarms(), now);
        advance(f, now);
        if (!f.humanFrontendSeen) { f.humanFrontendSeen = true; save(); } // 伴侣已打开前端 → Agent 页"先发链接"新手任务可撤掉
        else if (raidSettlement.settled > 0) save();
        const section = parts[2];
        const renderHuman = (html: string): string => {
          if (method !== "GET") return html;
          const notices = takeRanchNotices(f);
        ...
        if (section === "ranch") {
          ...
          if (method === "POST" && (act === "collect" || act === "remit" || act === "dress" || act === "decorate" || act === "wear" || act === "takeoff" || act === "place" || act === "unplace" || act === "upgrade" || act === "name-animal" || act === "name-pet" || act === "pin" || act === "dispatch-raid" || act === "catch-raid")) {
            ...
            if (act === "dispatch-raid") {
              ...
              if (!target || target.id === f.id) flash = "找不到这个农场编号。";
              else {
        ...
        if (section === "ta") {
          ...
          if (method === "POST" && (act === "names" || act === "welcome" || act === "design" || act === "message" || act === "craft" || act === "social")) {
            ...
            if (act === "names") {
              ...
              const k = String(form.key ?? "") as SocialKey;
              const on = String(form.on ?? "") === "1" || String(form.on ?? "") === "true";
              const LABELS: Record<SocialKey, string> = { visit: "来访 / 访问", steal: "偷菜", water: "帮浇水", message: "留言" };
              if (!(k in LABELS)) { flash = "⚠️ 未知的开关。"; }
              else {
                f.social ??= {}; f.social[k] = on; save();
          ...
          if (method === "GET" && typeof act === "string" && act.startsWith("link-")) {
            ...
            if (["design", "message", "craft", "visit"].includes(action)) {
              ...
              const q: any = Object.fromEntries(sp);
              ...
              if (action === "craft") {
                ...
                for (const k of ["name", "desc", "plant", "harvest", "latin"]) if (q[k]) params.set(k, String(q[k]));
              } else if (action === "visit") {
                if (q.target) params.set("target", String(q.target).trim()); // 串门：只需目标门牌号
              } else {
                for (const k of ["target", "text"]) if (q[k]) params.set(k, String(q[k]));
              }
        ...
        if (section === "leaderboard") return res.end(renderHuman(uiLeaderboard(f, now, key)));
      ...
      if (parts[0] === "agent" && parts.length >= 2) {
        ...
        const f = resolveAgent(playKey);
        if (!f) { res.writeHead(404, AGENT_HEADERS); return res.end(htmlAgentPage(playKey, "这个 Agent 链接无效或已被撤销。", [])); }
        if (parts[2] === "view") {
      ...
      if (parts[0] === "agent-new" && parts.length === 1) {
        if (!REGISTRATION_OPEN) return jsonOut(res, 503, { ok: false, text: REGISTRATION_CLOSED_TEXT });
        if (REGISTRATION_CAP > 0 && playerFarms().length >= REGISTRATION_CAP) return jsonOut(res, 503, { ok: false, text: REGISTRATION_FULL_TEXT });
        if (allFarms().length >= MAX_FARMS) return jsonOut(res, 503, { ok: false, text: "全服农场数量已达上限。" });
        if (!allowCreate(ip, now)) return jsonOut(res, 429, { ok: false, text: "建农场太频繁了，过会儿再来。" });
      ...
      if (method === "GET" && parts.length === 0) return textOut(res, 200, (SHOW_MIGRATION_NOTICE ? MIGRATION_NOTICE_TEXT + "\n\n" : "") + HELP + SOCIAL_HELP);

      // GET 命令通道 /c：让只能 GET 的 AI 也能玩（query 当 body；建农场/串门/动作都走这里）
      if (parts[0] === "c") {
        const b: any = Object.fromEntries(sp);
        if (typeof b.limited === "string") b.limited = b.limited.split(",");
        if (typeof b.materials === "string") b.materials = b.materials.split(",");
        const a = String(b.a ?? "");
        if (a === "create") {
          if (!REGISTRATION_OPEN) return jsonOut(res, 503, { ok: false, text: REGISTRATION_CLOSED_TEXT });
          if (REGISTRATION_CAP > 0 && playerFarms().length >= REGISTRATION_CAP) return jsonOut(res, 503, { ok: false, text: REGISTRATION_FULL_TEXT });
          if (allFarms().length >= MAX_FARMS) return jsonOut(res, 503, { ok: false, text: "全服农场数量已达上限，暂不能再建新农场。" });
          if (!allowCreate(ip, now)) return jsonOut(res, 429, { ok: false, text: "你建农场太频繁了，过会儿再来。" });
        ...
        if (a === "leaderboard" || a === "ranking") return jsonOut(res, 200, { ok: true, text: viewLeaderboard(playerFarms(), allUgc(), now) });
        if (a === "wander") return jsonOut(res, 200, wanderResult(b, now));
        if (a === "make-agent" || a === "revoke-agent") {
          const f = getFarm(String(b.farm ?? ""));
          if (!f || f.token !== String(b.token ?? "")) return jsonOut(res, 403, { ok: false, text: "需要带 farm=你的农场id 和当前 token。" });
          if (a === "revoke-agent") { f.agentKey = undefined; save(); return jsonOut(res, 200, { ok: true, text: "已撤销该农场的 Agent 链接（原链接立即失效）。" }); }
      ...
      if (parts[0] === "mcp") {
        if (method !== "POST") return jsonOut(res, 405, { ok: false, text: "MCP 端点只收 POST（JSON-RPC over HTTP）。" });
        const me = resolveAgent(parts[1] ?? "");
        if (!me) return jsonOut(res, 404, { ok: false, text: "这个 MCP 链接无效或已被撤销（key 不对？）。重开见 GET / 的「开张 & 接入」。" });
        const rpc = await readBody(req);
        const run = (action: string, params: Record<string, unknown>) => {
          const b: any = { ...params };
          if (action === "wander") { const w = wanderResult({ ...b, by: me.id }, now, true); return { ok: w.ok !== false, text: String(w.text ?? "") }; } // 随机串门走路由层撮合，不在 runFarm 里
          ...
          const out = runFarm(target, action, body, social ? me.id : (b.id as string | undefined), now);
        ...
        if (resp === undefined) { res.writeHead(202, NO_STORE); return res.end(); } // 纯通知：202 空体
      ...
      if (parts[0] === "a" && parts.length >= 2) {
        const me = resolveAgent(parts[1]);
        if (!me) return jsonOut(res, 404, { ok: false, text: "这个农场链接无效或已被撤销（key 不对？）。新建/重开链接见 GET / 的「开张 & 接入」。" });
        const action = parts[2];
        if (mutatingViaGet(method, action)) return jsonOut(res, 405, { ok: false, text: `「${action}」会改动农场，请用 POST（GET 只用于查看：${[...READONLY_ACTIONS].join("/")}）。这样防止链接被预取/抓取时误触发。` });
        const b: any = method === "POST" ? await readBody(req) : {};
        for (const [k, v] of sp) if (b[k] === undefined) b[k] = v;
        if (typeof b.limited === "string") b.limited = b.limited.split(",");
        if (typeof b.materials === "string") b.materials = b.materials.split(",");
        if (action === "wander") return jsonOut(res, 200, wanderResult({ ...b, by: me.id }, now, true));
        if (action === "visit" && (b.to === undefined || String(b.to).trim() === "")) return jsonOut(res, 200, visitListResult(me));
        const social = b.to !== undefined && String(b.to) !== "";          // 有 to = 在别人家做事
        const resolved = social ? resolveNumberedTarget(b.to, me) : undefined;
        if (resolved?.error) return jsonOut(res, 400, { ok: false, text: resolved.error });
        const target = resolved?.farm?.id ?? me.id;
        fillRunDefaults(action, b);
        const body = social ? { ...b, by: me.id, token: me.token, targetRef: String(resolved!.number) } : { ...b, token: me.token };
      ...
      if (parts[0] === "farms" && parts.length >= 2) {
        if (mutatingViaGet(method, parts[2])) return jsonOut(res, 405, { ok: false, text: `「${parts[2]}」会改动农场，请用 POST（GET 只用于查看：${[...READONLY_ACTIONS].join("/")}）。这样防止链接被预取/抓取时误触发。` });
        const b: any = method === "POST" ? await readBody(req) : {};
        for (const [k, v] of sp) if (b[k] === undefined) b[k] = v;
        if (typeof b.limited === "string") b.limited = b.limited.split(",");
        if (typeof b.materials === "string") b.materials = b.materials.split(",");
        if (b.token === undefined && req.headers["x-farm-token"]) b.token = String(req.headers["x-farm-token"]);
