...
import { cropById, getCrop, animalById, petById, accessoryById, decorationById, landTierByLevel, totalCropCount, cropsByCategory, qualities, materialById, recipes, expMaps, expEventById, expMapById, expDecorById, type Category, type Crop } from "./content.js";
...
import { buildLeaderboards, type BoardRow } from "./leaderboard.js";
...
import type { Farm, CodexEntry } from "./types.js";
...
const clock = (ms: number): string => new Date(ms).toLocaleTimeString("zh-CN", { timeZone: "Asia/Shanghai", hour: "2-digit", minute: "2-digit", hour12: false });
/** UTC+8 月-日 HH:MM（足迹时间戳用）。 */
const stamp = (ms: number): string => new Date(ms).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit", hour12: false });
...
const CAT_LABEL: Record<string, string> = { common: "普通", fantasy: "奇幻", limited: "限定", ugc: "自创" };
const RARITY_VAR: Record<string, string> = { N: "--N", R: "--R", SR: "--SR", SSR: "--SSR", SP: "--SP", OR: "--OR" };
/** 稀有度小色标，如 SR（带专属色边框）。 */
const rarityDot = (r: string): string =>
  `<span class="rdot" style="--c:var(${RARITY_VAR[r] ?? "--N"})">${esc(r)}</span>`;
/** 已收集的原创(ugc)物种数。 */
const ugcGot = (f: Farm): number =>
  Object.keys(f.codex).filter((id) => getCrop(id)?.category === "ugc").length;
/** 最近收录的若干条图鉴（codex 键按收集顺序插入，取末尾即最新）。 */
function recentCodex(f: Farm, n: number): { name: string; rarity: string }[] {
  return Object.keys(f.codex).slice(-n).reverse()
    .map((id) => getCrop(id)).filter((c): c is NonNullable<typeof c> => !!c)
    .map((c) => ({ name: c.name, rarity: c.rarity }));
}

/** 一座农场已收集官方物种数（普通+奇幻+限定）。 */
function codexGot(f: Farm): number {
  return codexCountByCategory(f, "common") + codexCountByCategory(f, "fantasy") + codexCountByCategory(f, "limited");
}
/** 在全服中按某打分函数排第几（1 起）。 */
function rankOf(farms: Farm[], me: Farm, score: (f: Farm) => number): number {
  const v = score(me);
  let r = 1;
  for (const o of farms) if (score(o) > v) r++;
  return r;
}
...
function nav(key: string, active: string): string {
...
const farmNames = (f: Farm): { ai: string; human: string } => ({ ai: f.aiName || "AI", human: f.humanName || "伴侣" });
const farmLabel = (f: Farm): string => `${f.name}（${f.aiName || "AI"}）`;

function page(title: string, key: string, active: string, body: string, names?: { ai: string; human: string }): string {
...
export function uiHumanNotices(html: string, notices: readonly string[]): string {
  if (!notices.length) return html;
...
const barFill = (pct: number, color: string) =>
  `<span style="width:${Math.max(0, Math.min(100, Math.round(pct)))}%;background:${color}"></span>`;

// ——————————————————————————————————————————————————————————————
// 🏡 农场主页 / 总览（打样）
// ——————————————————————————————————————————————————————————————
export function uiHome(f: Farm, now: number, key: string): string {
  advance(f, now);
  ...
  for (const farm of farms) advance(farm, now); // 广播读取此刻真实成熟状态，不写历史
  ...
  const cc = codexCountByCategory(f, "common"), fc = codexCountByCategory(f, "fantasy"),
        lc = codexCountByCategory(f, "limited"), oc = ugcGot(f);
  ...
  const plots = f.plots.map((p) => {
    if (!p.crop) return `<div class="plot empty"><span class="ico">🟫</span><span class="small">空地</span></div>`;
    const c = p.crop;
  ...
  if (f.shop.potionSet) shopBits.push(`🎁 药水套装（${f.shop.potionSet.qty} 瓶 / ${f.shop.potionSet.price} 金）`);
  if (f.shop.recipe) shopBits.push(`📜 配方【${esc(cropById.get(f.shop.recipe)?.name ?? f.shop.recipe)}】`);
  if (s.limited?.length) shopBits.push(`🎏 限定：${s.limited.map((l) => esc(l.name)).join("、")}`);
  ...
  const trailRow = (e: NonNullable<Farm["trail"]>[number]): string => {
    const who = `<b>${esc(e.by || "有人")}</b>`;
    const plot = e.plotId != null ? ` ${e.plotId} 号地` : "";
    const text =
      e.kind === "watered" ? `💧 ${who} 帮${plot}浇了水`
...
// ——————————————————————————————————————————————————————————————
export function uiRanch(f: Farm, now: number, key: string, flash?: string): string {
  advance(f, now);
  ...
  const pinnedSet = new Set<string>(f.ranch?.pinned ?? []);
  // 📌 pin 开关：被 pin 的动物/宠物才会随机出现在小克农场的氛围句里（都没 pin=全部随机）
  const pinBtn = (kindId: string) => {
  ...
  for (const a of list) { const k = animalById.get(a.kindId); if (k) pendingGross += Math.round(a.pending * k.producePrice * (1 + ((a.level ?? 1) - 1) * RANCH_LEVEL_INCOME_STEP)); }
  ...
  const fmtTime = (ms: number) => new Date(ms).toLocaleString("zh-CN", { timeZone: "Asia/Shanghai", month: "2-digit", day: "2-digit", hour: "2-digit", minute: "2-digit" });
  ...
  if (!list.length && !incoming.length) {
    ...
    const ownRows = list.map((a, i) => {
      ...
      const upBtn = !k ? "" : (lvl >= RANCH_ANIMAL_MAX_LEVEL)
        ...
        : (() => { const cost = animalUpgradeCost(k, lvl); const can = coins >= cost;
  ...
  const raidHistoryRows = raidHistory.length
    ? raidHistory.map((entry) => {
        const result = entry.status === "active"
          ...
          : entry.status === "caught"
            ? `<b style="color:var(--SP)">被抓 -${num(entry.coins ?? 0)} 金</b>`
            : `<b style="color:var(--leaf-deep)">+${num(entry.coins ?? 0)} 金</b>`;
  ...
  const raidHistoryCard = `<details class="card raid-history"><summary><span>🥷 今日已偷 <b>${num(todayRaidIncome)}/${RANCH_RAID_DAILY_CAP}</b></span></summary>
  ...
  const accOffers = (shop?.acc ?? []).map((id) => accessoryById.get(id)).filter(Boolean) as { id: string; name: string; price: number }[];
  const decoOffers = (shop?.decor ?? []).map((id) => decorationById.get(id)).filter(Boolean) as { id: string; name: string; price: number }[];
  ...
  const decorName = (id: string) => (decorationById.get(id) ?? expDecorById.get(id))?.name ?? id;
  const decorVisit = (id: string) => (decorationById.get(id) ?? expDecorById.get(id) as { visitLine?: string })?.visitLine;
  const decorSrc = (id: string) => {
  ...
  for (const id of ranch?.wardrobe ?? []) wdCount.set(id, (wdCount.get(id) ?? 0) + 1);
  ...
  // 装饰·仓库里（未摆，可摆上）
  const storeRows = (ranch?.decorStore ?? []).map((id) =>
    `<div class="line small" style="flex-wrap:wrap"><span>🏡 <b>${esc(decorName(id))}</b>　${decorSrc(id)}</span>
...
// ——————————————————————————————————————————————————————————————
export function uiTa(f: Farm, now: number, key: string, flash?: string): string {
  advance(f, now);
  ...
  const sOn = (k: "visit" | "steal" | "water" | "message") => f.social?.[k] !== false;
  const visitOpen = sOn("visit");
  const sRow = (k: "visit" | "steal" | "water" | "message", title: string, onTip: string, offTip: string) => {
    const on = sOn(k);
  ...
  if (matTotal < 3) {
    ...
    const sel = (n: string) => `<select class="inp" name="${n}" style="width:auto" required>${opts}</select>`;
...
// ——————————————————————————————————————————————————————————————
const EXP_TYPE_LABEL: Record<string, string> = { story: "剧情", drop: "掉落", choice: "分支", encounter: "奇遇", combat: "⚔️战斗" };
function expBagPreview(exp: NonNullable<Farm["expedition"]>): string {
  let coins = 0, silver = 0, potion = 0; const decor: string[] = [];
  for (const d of exp.bag) {
    if (d.t === "coins") coins += d.n ?? 0; else if (d.t === "silver") silver += d.n ?? 0;
    else if (d.t === "potion") potion += d.n ?? 0; else if (d.t === "decor") decor.push(expDecorById.get(d.id!)?.name ?? "装饰");
  }
  const parts: string[] = [];
  if (coins) parts.push(`${coins}金`); if (silver) parts.push(`${silver}银`);
  if (potion) parts.push(`药水×${potion}`); if (decor.length) parts.push(`🏡${decor.join("、")}`);
  return parts.length ? parts.join("、") : "空";
}

export function uiExpedition(f: Farm, now: number, key: string, flash?: string): string {
  advance(f, now);
  ...
  const mapBlocks = expMaps.map((map) => {
    const evs = map.events.map((id) => expEventById.get(id)).filter((e): e is NonNullable<typeof e> => !!e);
    const got = evs.filter((e) => seen.has(e.id)).length;
    const micon = "🗺️";
    const mcolor = "var(--leaf-deep)";
    // 未发现的秘境：连名字带内容整块盖住、不可展开；去过至少一格才解锁，露出真名与格子。
    if (got === 0) return `<div class="card" style="padding:10px 12px;font-family:var(--serif);color:#c2b89e;letter-spacing:3px;font-weight:600">${micon} ？？？　<span class="small" style="font-weight:400;letter-spacing:0;color:#c2b89e">未解锁</span></div>`;
    const rows = evs.map((e) => {
      if (!seen.has(e.id)) return `<div style="${rowStyle};color:#c2b89e;letter-spacing:3px">？？？</div>`;
      const detail: string[] = [`<div style="white-space:pre-wrap">${esc(e.story)}</div>`];
      if (e.options?.length) detail.push(`<div class="small muted" style="margin-top:6px">岔路：<br>${e.options.map((o) => `▸ ${o.key}. ${esc(o.label)}`).join("<br>")}</div>`);
      if (e.type === "combat") {
        const bits = [`敌人：${esc(e.foe ?? "")}`];
        if (e.win?.text) bits.push(`胜 → ${esc(e.win.text)}`);
        if (e.lose?.text) bits.push(`负 → ${esc(e.lose.text)}`);
...
// ——————————————————————————————————————————————————————————————
const RAR_RANK: Record<string, number> = { N: 0, R: 1, SR: 2, SSR: 3, SP: 4, OR: 5 };
const qualityName = (tier: number): string => qualities.find((q) => q.tier === tier)?.name ?? `品${tier}`;
const byRarityThenName = (a: Crop, b: Crop): number =>
  (RAR_RANK[a.rarity] - RAR_RANK[b.rarity]) || a.name.localeCompare(b.name, "zh");
const fmtDate = (ms?: number): string =>
  ms ? new Date(ms).toLocaleDateString("zh-CN", { timeZone: "Asia/Shanghai", year: "numeric", month: "2-digit", day: "2-digit" }) : "早期收录";
/** 拼 data-* 属性（空值不输出，值统一转义）。*/
const da = (k: string, v?: string | number | null): string =>
  (v === undefined || v === null || v === "") ? "" : ` data-${k}="${esc(v)}"`;

/** 一枚标本位。entry 有则「已收录」（亮），无则灰（mine=true 时为「已设计待收获」，不灰）。
 *  已收录 / 我的 两类带 data-* → 可点开细节弹窗；未解锁的灰位不可点。 */
function specTile(c: Crop, entry: CodexEntry | undefined, opts: { mine?: boolean; by?: string; cat?: string; key?: string; starred?: boolean; anchor?: string } = {}): string {
...
export function uiCodex(f: Farm, now: number, key: string, flash?: string): string {
  advance(f, now);
  const flashHtml = flash ? `<div class="flash">${esc(flash)}</div>` : "";

  // 官方三类：全集铺位
  const official: { id: string; emoji: string; label: string; cat: Category }[] = [
  ...
  const others = Object.keys(f.codex)
    .map((id) => getCrop(id))
    .filter((c): c is Crop => !!c && c.category === "ugc" && c.designerId !== f.id)
  ...
  const favLabel = (c: Crop): string =>
    c.category === "ugc" ? (c.designerId === f.id ? "我的" : "原创")
    : c.category === "common" ? "普通" : c.category === "fantasy" ? "奇幻" : "限定";
  const favs = (f.starred ?? [])
    .map((id) => getCrop(id))
    .filter((c): c is Crop => !!c && !c.banned)
...
function lbRow(rank: number, name: string, value: number, unit: string, max: number, isMe: boolean, by?: string, off?: boolean, title?: string, code?: string, byCode?: string, valuePrefix = ""): string {
...
export function uiLeaderboard(f: Farm, now: number, key: string): string {
  advance(f, now);
  ...
  const today = currentDayIndex(now);
  type Def = { icon: string; title: string; unit: string; sub?: string; rows: BoardRow[]; score: (x: Farm) => number; valuePrefix?: string };
  const defs: Def[] = [
  ...
  const mkCard = (d: Def) => {
    ...
    let foot = "";
    if (meVal > 0 && !inRows) foot = lbRow(meRank - 1, f.name, meVal, d.unit, max, true, undefined, true, equippedTitle(f)?.name, f.id, undefined, d.valuePrefix);
    else if (meVal <= 0) foot = `<div class="lbnote">${aiDisp}还没上这个榜～</div>`;
...
export function uiTodo(f: Farm, key: string, section: string): string {
  const names: Record<string, string> = { leaderboard: "排行榜" };
...
export function uiInvalid(): string {
