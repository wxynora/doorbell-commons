...
export interface Leaderboards {
  ...
  todayRaidIncome: BoardRow[]; // 摸金榜（今日动物偷金币实际到账）
  todayRaidLoss: BoardRow[];   // 漏财榜（今日动物偷金币玩法实际损失）
...
export function viewLeaderboard(farms: Farm[], ugc: Crop[], now: number): string {
  ...
  const fmt = (rows: BoardRow[], unit: string, valuePrefix = "") =>
    rows.length ? rows.map((r, i) => `  ${i + 1}. ${r.title ? `✧${r.title}✧` : ""}${r.name} · ${r.code} — ${valuePrefix}${r.value}${unit}`).join("\n") : "  （暂无）";
